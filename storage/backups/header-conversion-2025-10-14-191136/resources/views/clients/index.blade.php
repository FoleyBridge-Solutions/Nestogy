...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 dark:bg-gray-800 shadow rounded-lg">
        <div class="border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
            <nav class="-mb-px flex space-x-8 px-6" aria-label="Tabs">
                <a href="{{ route('clients.index') }}" class="{{ !request('lead') ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:text-gray-300 hover:border-gray-300 dark:border-gray-600 dark:border-gray-600' }} whitespace-nowrap py-6 px-1 border-b-2 font-medium text-sm">
                    Customers
                </a>
                <a href="{{ route('clients.leads') }}" class="{{ request('lead') ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-300 dark:text-gray-300 hover:border-gray-300 dark:border-gray-600 dark:border-gray-600' }} whitespace-nowrap py-6 px-1 border-b-2 font-medium text-sm">
                    Leads
                </a>
            </nav>
        </div>
    ...
    <!-- DataTable Container -->
    <div class="bg-white dark:bg-gray-800 dark:bg-gray-800 shadow overflow-hidden rounded-lg">
        <div class="px-6 py-8 sm:px-6 border-b border-gray-200 dark:border-gray-700 dark:border-gray-700">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <!-- Export -->
                    <a href="{{ route('clients.export.csv', ['lead' => request('lead')]) }}" class="inline-flex items-center px-6 py-2 border border-gray-300 dark:border-gray-600 dark:border-gray-600 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 dark:text-gray-300 dark:text-gray-300 bg-white dark:bg-gray-800 dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900 dark:hover:bg-gray-700 dark:bg-gray-900">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-6">
            <table id="clientsTable" class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50 dark:bg-gray-900 dark:bg-gray-900">
                    <tr>
                        <th scope="col-span-12" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                        <th scope="col-span-12" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Email</th>
                        <th scope="col-span-12" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Phone</th>
                        ...
                        <th scope="col-span-12" class="px-6 py-6 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tags</th>
...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-8 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800 dark:bg-gray-800">
        <div class="mt-6 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white dark:text-white mt-2">Delete Client</h3>
            ...
            <div class="items-center px-6 py-6">
                <button id="confirmDelete" class="px-6 py-2 bg-red-500 text-white text-base font-medium rounded-md w-24 mr-2 hover:bg-red-600">Delete</button>
...
<div id="convertModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-8 border w-96 shadow-lg rounded-md bg-white dark:bg-gray-800 dark:bg-gray-800">
        <div class="mt-6 text-center">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-white dark:text-white mt-2">Convert Lead to Customer</h3>
            <div class="mt-2 px-7 py-6">
                <p class="text-sm text-gray-500">Are you sure you want to convert this lead to a customer?</p>
            </div>
            <div class="items-center px-6 py-6">
                <button id="confirmConvert" class="px-6 py-2 bg-green-500 text-white text-base font-medium rounded-md w-24 mr-2 hover:bg-green-600">Convert</button>
                <button onclick="closeConvertModal()" class="px-6 py-2 bg-gray-500 text-white text-base font-medium rounded-md w-24 hover:bg-gray-600">Cancel</button>
            </div>
        </div>
    </div>
</div>
        ...
        columns: [
            {
                data: 'name',
                name: 'name',
                render: function(data, type, flex flex-wrap) {
                    let html = '<div class="flex items-center">';
                    html += '<div class="flex-shrink-0 h-10 w-10">';
                    if (flex flex-wrap.avatar) {
                        html += '<img class="h-10 w-10 rounded-full" src="' + flex flex-wrap.avatar + '" alt="' + data + '">';
                    } else {
                        html += '<div class="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center">';
                        html += '<span class="text-sm font-medium text-gray-700 dark:text-gray-300 dark:text-gray-300">' + data.substring(0, 2).toUpperCase() + '</span>';
                        html += '</div>';
                    }
                    html += '</div>';
                    html += '<div class="ml-4">';
                     html += '<div class="text-sm font-medium text-gray-900 dark:text-white dark:text-white">';
                     html += data;
                     html += '</div>';
                    if (flex flex-wrap.company) {
                        html += '<div class="text-sm text-gray-500">' + flex flex-wrap.company + '</div>';
                    }
                    html += '</div>';
                    html += '</div>';
                    return html;
            ...
            {data: 'phone', name: 'phone'},
            {
                ...
                render: function(data) {
                    return '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 dark:bg-gray-800 text-gray-800 dark:text-gray-200 dark:text-gray-200">' + 
            ...
            },
            {
                ...
                render: function(data, type, flex flex-wrap) {
                    let html = '<div class="flex items-center space-x-2">';
                     @if($selectedClient)
                         if ({{ $selectedClient->id }} !== flex flex-wrap.id) {
                             html += '<button onclick="selectClient(' + flex flex-wrap.id + ', \'' + flex flex-wrap.name + '\')" class="text-green-600 hover:text-green-900">Select</button>';
                         } else {
                             html += '<span class="text-green-600 font-medium">Current</span>';
                         }
                     @else
                         html += '<button onclick="selectClient(' + flex flex-wrap.id + ', \'' + flex flex-wrap.name + '\')" class="text-blue-600 hover:text-blue-900 font-medium">Select</button>';
                     @endif
                     // Edit functionality removed as per requirements
                    if (flex flex-wrap.lead) {
                        html += '<button onclick="convertLead(' + flex flex-wrap.id + ')" class="text-green-600 hover:text-green-900">Convert</button>';
                    }
                    html += '<button onclick="deleteClient(' + flex flex-wrap.id + ')" class="text-red-600 hover:text-red-900">Delete</button>';
                    html += '</div>';
                    return html;
                }
            }
