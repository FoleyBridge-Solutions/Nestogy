        ...
        <flux:header class="px-4! w-full bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
            ...
            <flux:navbar class="hidden sm:flex">
                @guest
                    ...
                    @if (Route::has('signup.form'))
                        <flux:button href="{{ route('signup.form') }}" variant="primary">
                            {{ __('Start Free Trial') }}
                        </flux:button>
                    @endif
                @endguest
            ...
            <div class="sm:hidden">
                <flux:dropdown>
                    ...
                    <flux:navmenu>
                        @guest
                            <flux:navmenu.item href="{{ route('login') }}">
                                {{ __('Login') }}
                            </flux:navmenu.item>
                            @if (Route::has('signup.form'))
                                <flux:navmenu.item href="{{ route('signup.form') }}">
                                    {{ __('Start Free Trial') }}
                                </flux:navmenu.item>
                            @endif
                        @endguest
                        @auth
                            <flux:navmenu.item href="{{ route('dashboard') }}">
                                {{ __('Dashboard') }}
                            </flux:navmenu.item>
                        @endauth
                    </flux:navmenu>
