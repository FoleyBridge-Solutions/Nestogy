...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Overview of all contract activities and performance</p>
        </div>
        <div class="d-flex gap-2">
            <button type="button" class="btn btn-outline-secondary" id="customize-dashboard">
    ...
    <!-- Quick Stats Row -->
    <div class="row mb-4" id="stats-widgets">
        @foreach($statsWidgets as $widget)
            <div class="col-xl-3 col-md-6 mb-3" data-widget-id="{{ $widget['id'] }}">
                    ...
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <div class="widget-actions">
                            @if($widget['filterable'] ?? false)
                                <button type="button" class="btn btn-sm btn-ghost" data-bs-toggle="dropdown">
                            ...
                            @if($widget['expandable'] ?? false)
                                <button type="button" class="btn btn-sm btn-ghost widget-expand" title="Expand">
                                    <i class="fas fa-expand"></i>
                                </button>
                            @endif
                            <button type="button" class="btn btn-sm btn-ghost widget-refresh" title="Refresh">
                            ...
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end">
                                    ...
                                    <li><a class="dropdown-item text-danger widget-remove" href="#">Remove</a></li>
                                </ul>
                            </div>
                        </div>
                    </div>
    ...
    <div class="text-center py-4" id="add-widget-area">
        <button type="button" class="btn btn-outline-primary btn-lg" id="add-widget-btn">
            <i class="fas fa-plus"></i> Add Widget
        </button>
        <p class="text-muted mt-2">Customize your dashboard by adding widgets</p>
    </div>
</div>

<!-- Widget Customization Modal -->
<div class="modal fade" id="customize-modal" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-3">
                        <div class="nav flex-column nav-pills" id="widget-categories">
                            <button class="nav-link active" data-category="overview">Overview</button>
                            <button class="nav-link" data-category="financial">Financial</button>
                            <button class="nav-link" data-category="performance">Performance</button>
                            <button class="nav-link" data-category="analytics">Analytics</button>
                            <button class="nav-link" data-category="custom">Custom</button>
                        </div>
                    </div>
                    <div class="col-md-9">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-primary" id="save-dashboard">Save Changes</button>
            </div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="widget-config-modal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary" id="save-widget-config">Save Configuration</button>
            </div>
        </div>
    </div>
</div>
...
<style>
    ...
    .widget-actions .btn-ghost:hover {
        color: #495057;
        background-color: #f8f9fa;
    }
...
class ContractDashboard {
    ...
    initializeWidget(element, widget) {
        ...
        if (widget.grid_position) {
            ...
            element.style.gridRow = pos.row ? `${pos.row} / span ${pos.height || 1}` : '';
        }
    }
    ...
    loadWidgetLibrary(category) {
        ...
        fetch(`/api/contracts/dashboard/widget-library?category=${category}`)
            ...
            .catch(error => {
                ...
                library.innerHTML = '<div class="text-center text-danger">Failed to load widgets</div>';
            });
    }
    ...
    renderWidgetLibrary(widgets) {
        ...
        library.innerHTML = widgets.map(widget => `
            <div class="widget-library-item" data-widget-type="${widget.type}">
                ...
                <div class="mt-2">
                    ${widget.tags.map(tag => `<span class="badge bg-light text-dark me-1">${tag}</span>`).join('')}
                </div>
            </div>
        `).join('');
