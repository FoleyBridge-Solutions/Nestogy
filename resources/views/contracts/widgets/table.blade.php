...
<div class="widget-table-container">
    ...
    @else
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-sm widget-table" id="widget-table-{{ $widget['id'] }}">
                                    ...
                                    @elseif($column['type'] === 'actions')
                                        <div class=" 
                                            @foreach($column['actions'] ?? [] as $action)
                                                <flux:button variant="primary" href="{{ str_replace('{id}', $flex flex-wrap['id'] ?? '', $action['url'] ?? '#') }}" class="$action['color'] ?? 'secondary' }}" title="{{ $action['label'] }}"
