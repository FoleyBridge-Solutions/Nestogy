            ...
            <div class="row align-items-center">
                <div class="col-md-6">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" class="form-control" placeholder="Search..." id="widget-search-{{ $widget['id'] }}">
                    </div>
                </div>
                @if($showPagination)
                            ...
                            @foreach($columns as $column)
                                ...
                                <td class="{{ $column['align'] ?? '' }}" 
                                    ...
                                    @elseif($column['type'] === 'avatar')
                                        <div class="d-flex align-items-center">
                                            <img src="{{ $row[$column['avatar_key'] ?? 'avatar'] ?? 'https://ui-avatars.com/api/?name=' . urlencode($value) }}" 
                                                 class="rounded-circle me-2" width="24" height="24" alt="{{ $value }}">
                                            {{ $formatted }}
                                        </div>
                                    ...
                                    @elseif($column['type'] === 'link')
                                        <a href="{{ str_replace('{id}', $row['id'] ?? '', $column['url'] ?? '#') }}" 
...
@if(!empty($tableData))
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateSortIcons(activeHeader, direction) {
            ...
            icon.className = `fas fa-sort-${direction === 'asc' ? 'up' : 'down'} sort-icon ms-1`;
        }
        @endif
