...
<div class="container mx-auto-fluid">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400">Monitor IP addresses, threat levels, and geographic data</p>
        </div>
    ...
    <div class="flex flex-wrap -mx-4 mb-6">
        <div class="xl:w-1/4 flex-1 px-6-md-6 mb-6">
            ...
            <form method="GET" action="{{ route('security.dashboard.ip-intelligence') }}">
                <div class="flex flex-wrap -mx-4">
                    <div class="flex-1 px-6-md-3">
                        <label for="threat_level" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Threat Level</label>
                        <select class="block w-full px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" id="threat_level" name="threat_level">
                    ...
                    <div class="flex-1 px-6-md-3">
                        ...
                        <input type="text" class="block w-full px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm" id="country" name="country" 
                    ...
                    <div class="flex-1 px-6-md-3 flex items-end">
                        ...
                        <a href="{{ route('security.dashboard.ip-intelligence') }}" class="btn px-6 py-2 font-medium rounded-md transition-colors-outline-secondary">
    ...
    <div class="flex flex-wrap -mx-4 mb-6">
        <div class="xl:w-1/2 flex-1 px-6-lg-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden shadow">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-header py-6">
                    <h6 class="m-0 font-bold text-blue-600 dark:text-blue-400">Threat Level Distribution</h6>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-body">
        ...
        <div class="xl:w-1/2 flex-1 px-6-lg-6">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden shadow">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-header py-6">
                    <h6 class="m-0 font-bold text-blue-600 dark:text-blue-400">Threat Level Breakdown</h6>
                </div>
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-body">
                    @foreach(['low' => 'success', 'medium' => 'warning', 'high' => 'danger', 'critical' => 'dark'] as $level => $color)
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            <span class="badge inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium-{{ $color }}">{{ ucfirst($level) }}</span>
                        </div>
                        <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-body">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div>
                                        @if($ipLog->country_code)
                                            <span class="flag-icon flag-icon-{{ strtolower($ipLog->country_code) }} mr-2"></span>
                                        @endif
                                        <strong>{{ $ipLog->getLocationString() }}</strong>
                                        @if($ipLog->timezone)
                                            <br><small class="text-gray-600 dark:text-gray-400">{{ $ipLog->timezone }}</small>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    @endphp
                                    <span class="badge inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium-{{ $threatColors[$ipLog->threat_level] ?? 'secondary' }}">
                                        {{ ucfirst($ipLog->threat_level) }}
                                    </span>
                                    <br><small class="text-gray-600 dark:text-gray-400">Score: {{ $ipLog->getThreatScore() }}/100</small>
                                </td>
                                <td>
                                    <div>
                                        @if($ipLog->is_vpn)
                                            <span class="badge inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium-warning mr-1">VPN</span>
                                        @endif
                                        @if($ipLog->is_proxy)
                                            <span class="badge inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium-warning mr-1">Proxy</span>
                                        @endif
                                        @if($ipLog->is_tor)
                                            <span class="badge inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium-danger mr-1">Tor</span>
                                        @endif
                                        @if(!$ipLog->is_vpn && !$ipLog->is_proxy && !$ipLog->is_tor)
                                            <span class="text-gray-600 dark:text-gray-400">None detected</span>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    <div class="text-center">
                                        ...
                                        <br><small class="text-gray-600 dark:text-gray-400">lookups</small>
                                    </div>
                                </td>
                                <td>
                                    <div>
                                        ...
                                        @if($ipLog->last_lookup_at)
                                            <br><small class="text-gray-600 dark:text-gray-400">{{ $ipLog->last_lookup_at->diffForHumans() }}</small>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    <div class=" px-6 py-2 font-medium rounded-md transition-colors-group-sm">
                                        <flux:button variant="ghost" color="sky" class="px-6 py-2 font-medium rounded-md transition-colors-sm" onclick="showIpDetails({{ json_encode($ipLog) }})">
                                        ...
                                        @else
                                            <button class="btn border border-green-600 text-green-600 hover:bg-green-50 px-6 py-2 font-medium rounded-md transition-colors-sm" 
                                                    onclick="unblockIpAddress('{{ $ipLog->ip_address }}')">
                                                <i class="fas fa-check"></i> Unblock
                                            </button>
                                        @endif
                                        <button class="btn border border-blue-600 text-blue-600 hover:bg-blue-50 px-6 py-2 font-medium rounded-md transition-colors-sm" 
            ...
            @else
                <div class="text-center py-8">
                    <i class="fas fa-globe fa-4x text-gray-600 dark:text-gray-400 mb-6"></i>
                    <h4>No IP Data Found</h4>
                    <p class="text-gray-600 dark:text-gray-400">No IP lookup data found matching your criteria.</p>
                </div>
            @endif
        </div>
    </div>
...
<!-- IP Details Modal -->
<div class="fixed inset-0 z-50 overflow-y-auto fade" id="ipDetailsModal" tabindex="-1">
    <div class="flex items-center justify-center min-h-screen fixed inset-0 z-50 overflow-y-auto-lg">
        <div class="fixed inset-0 z-50 overflow-y-auto-content">
            <div class="fixed inset-0 z-50 overflow-y-auto-header">
                <h5 class="fixed inset-0 z-50 overflow-y-auto-title">IP Address Details</h5>
...
<div class="fixed inset-0 z-50 overflow-y-auto fade" id="unblockIpModal" tabindex="-1">
    <div class="fixed inset-0 z-50 overflow-y-auto-dialog">
        <div class="fixed inset-0 z-50 overflow-y-auto-content">
            <div class="fixed inset-0 z-50 overflow-y-auto-header">
                <h5 class="fixed inset-0 z-50 overflow-y-auto-title">Unblock IP Address</h5>
                <button type="button" class="px-6 py-2 font-medium rounded-md transition-colors-close" ></button>
            </div>
            <form action="{{ route('security.dashboard.unblock-ip') }}" method="POST">
                ...
                <div class="fixed inset-0 z-50 overflow-y-auto-footer">
                    <button type="button" class="btn px-6 py-2 font-medium rounded-md transition-colors-secondary" >Cancel</button>
                    <button type="submit" class="btn px-6 py-2 font-medium rounded-md transition-colors-success">Unblock IP</button>
                </div>
            </form>
        </div>
    </div>
    ...
    const content = `
        <div class="flex flex-wrap -mx-4">
            <div class="flex-1 px-6-md-6">
                <h6 class="text-blue-600 dark:text-blue-400">Basic Information</h6>
                ...
                ${ipLog.latitude && ipLog.longitude ? `
                    <h6 class="text-blue-600 dark:text-blue-400 mt-6">Geographic Coordinates</h6>
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 min-w-full divide-y divide-gray-200 dark:divide-gray-700-sm">
            ...
            <div class="flex-1 px-6-md-6">
                <h6 class="text-blue-600 dark:text-blue-400">Threat Analysis</h6>
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 min-w-full divide-y divide-gray-200 dark:divide-gray-700-sm">
                    <tr><td><strong>Threat Level:</strong></td><td><span class="badge inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium-${getThreatColor(ipLog.threat_level)}">${ipLog.threat_level}</span></td></tr>
                    <tr><td><strong>Threat Score:</strong></td><td>${ipLog.threat_score || 'N/A'}/100</td></tr>
                    <tr><td><strong>VPN Detected:</strong></td><td>${ipLog.is_vpn ? '<span class="text-red-600 dark:text-red-400">Yes</span>' : '<span class="text-green-600 dark:text-green-400">No</span>'}</td></tr>
                    <tr><td><strong>Proxy Detected:</strong></td><td>${ipLog.is_proxy ? '<span class="text-red-600 dark:text-red-400">Yes</span>' : '<span class="text-green-600 dark:text-green-400">No</span>'}</td></tr>
                    <tr><td><strong>Tor Detected:</strong></td><td>${ipLog.is_tor ? '<span class="text-red-600 dark:text-red-400">Yes</span>' : '<span class="text-green-600 dark:text-green-400">No</span>'}</td></tr>
                </table>

                <h6 class="text-blue-600 dark:text-blue-400 mt-6">Lookup Information</h6>
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 min-w-full divide-y divide-gray-200 dark:divide-gray-700-sm">
