        ...
        <x-slot name="actions">
            <div class="flex gap-3">
                ...
                <a href="{{ route('client.quotes.show', $quote) }}" 
                   target="_blank"
                   class="inline-flex items-center px-4 py-2 border border-purple-300 rounded-md shadow-sm text-sm font-medium text-purple-700 bg-white hover:bg-purple-50">
                    <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
function deleteQuote(quoteId, quoteNumber) {
    if (confirm(`Are you sure you want to delete quote ${quoteNumber}? This action cannot be undone.`)) {
        ...
        .then(data => {
            if (data.success) {
                // Redirect to quotes index page after successful deletion
