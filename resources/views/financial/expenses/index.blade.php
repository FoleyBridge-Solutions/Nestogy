...
<div class="container mx-auto mx-auto px-4 mx-auto px-4 mx-auto px-6 py-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            ...
                            <dd class="text-lg font-medium text-gray-900">{{ $stats['pending_approval_count'] ?? 0 }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white overflow-hidden shadow rounded-lg">
            <div class="p-8">
                <div class="flex items-center">
                    ...
                    <div class="ml-5 w-0 flex-1">
                        <dl>
                            <dt class="text-sm font-medium text-gray-500 truncate">Billable</dt>
                            <dd class="text-lg font-medium text-gray-900">${{ number_format($stats['billable_amount'] ?? 0, 2) }}</dd>
                        </dl>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white shadow rounded-lg mb-6">
        <div class="p-6">
            <form method="GET" action="{{ route('financial.expenses.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                ...
                <div>
                    <label for="billable" class="block text-sm font-medium text-gray-700">Billable</label>
                    <select name="billable" id="billable" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md">
        ...
        <div class="px-6 py-8 sm:p-6">
                            ...
                            @foreach($expenses as $expense)
                                <tr>
                                    ...
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        ...
                                        @if($expense->is_billable && $expense->total_billable_amount)
                                            <div class="text-xs text-green-600">
                                                Billable: ${{ number_format($expense->total_billable_amount, 2) }}
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        <div class="text-sm text-gray-900">{{ $expense->submittedBy->name }}</div>
                                        @if($expense->client)
                                            <div class="text-sm text-gray-500">{{ $expense->client->name }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-6 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $expense->status_badge_class }}">
                                            {{ $statuses[$expense->status] ?? $expense->status }}
                                        </span>
                                        @if($expense->status === 'pending_approval' && auth()->user()->hasRole(['manager', 'admin']))
                                            <div class="mt-1 flex space-x-1">
                                                <form method="POST" action="{{ route('financial.expenses.approve', $expense) }}" class="inline">
                                                    @csrf
                                                ...
                                                <button type="button" class="text-xs text-red-600 hover:text-red-900"
                                        ...
                                        @if($expense->receipt_path)
                                            <div class="flex items-center mt-1">
                                                <svg class="w-4 h-4 text-gray-400 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    ...
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2.586-2.586a2 2 0 112.828 0L12 13"></path>
                                                </svg>
                                    ...
                                    <td class="px-6 py-6 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('financial.expenses.show', $expense) }}" 
                                               class="text-indigo-600 hover:text-indigo-900">View</a>
                                            @can('update', $expense)
                                                @if(in_array($expense->status, ['draft', 'rejected']))
                                                    <a href="{{ route('financial.expenses.edit', $expense) }}" 
                                                       class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                                @endif
                                            @endcan
                                            @if($expense->receipt_path)
                                                <a href="{{ route('financial.expenses.receipt', $expense) }}" 
                                                   class="text-indigo-600 hover:text-indigo-900" target="_blank">Receipt</a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
            ...
            @else
                <div class="text-center py-6">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Get started by creating a new expense report.</p>
                    <div class="mt-6">
                        <a href="{{ route('financial.expenses.create') }}" 
...
<div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
    <div class="relative top-20 mx-auto p-8 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-6 text-center">
            <h3 class="text-lg font-medium text-gray-900">Reject Expense</h3>
            <form id="rejectForm" method="POST" class="mt-6">
                ...
                <div class="flex justify-end space-x-3 mt-6">
                    <button type="button" onclick="closeRejectModal()" 
                            class="px-6 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">Cancel</button>
                    <button type="submit" 
                            class="px-6 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Reject</button>
                </div>
            </form>
        </div>
    </div>
...
function openRejectModal(expenseId) {
    document.getElementById('rejectForm').action = `/financial/expenses/${expenseId}/reject`;
