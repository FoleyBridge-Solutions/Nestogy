    ...
    <div class="max-w-7xl mx-auto px-6">
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700">
            <form action="{{ route('financial.contracts.store') }}" method="POST" @submit="handleSubmission">
                ...
                <div x-show="currentStep === 1" x-transition class="px-6 pb-4">
                    <div>
                        <div>
                            ...
                            <div x-show="selectedTemplate" x-transition class="mt-6 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-700 p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div class="space-y-6">
                                <!-- Contract Title -->
                                <div class="group">
                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Contract Title *</label>
                                    <div class="relative">
                                        <input type="text" name="title" x-model="form.title" required
                                               class="w-full px-6 py-6 text-lg border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-400 transition-all duration-200"
                                ...
                                <!-- Contract Type -->
                                <div class="group">
                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Contract Type *</label>
                                    <div class="relative">
                                ...
                                <div class="group">
                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Client *</label>
                                    <div class="relative">
                            ...
                            <div class="space-y-6">
                                ...
                                <div class="space-y-4">
                                    <!-- Start Date -->
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Start Date *</label>
                                        <input type="date" name="start_date" x-model="form.start_date" required
                                               class="w-full px-6 py-6 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-400 transition-all duration-200">
                                        ...
                                        <div class="grid grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input type="date" name="end_date" x-model="form.end_date"
                                                       ...
                                                       class="w-full px-6 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-400 transition-all duration-200">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Currency</label>
                                    ...
                                    <div>
                                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Payment Terms</label>
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-6">Description</label>
                                    <textarea name="description" x-model="form.description" rows="4"
                                              class="w-full px-6 py-6 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:focus:border-blue-400 transition-all duration-200 resize-none"
                ...
                <input type="hidden" name="sla_terms" :value="JSON.stringify(slaTerms)">
...
function contractWizard() {
    return {
        ...
        // SLA Terms for auto-assignment settings
        slaTerms: {
        ...
        selectTemplate(template) {
            ...
            if (template) {
                ...
                // Set billing model from template
                if (template.billing_model) {
                    this.pricingSchedule.billingModel = template.billing_model;
                    console.log('Set billing model from template:', template.billing_model);
                }
                
                // Set supported asset types based on template type
                this.infrastructureSchedule.supportedAssetTypes = this.getSupportedAssetTypesForTemplate(template);
                console.log('Set supported asset types for template:', this.infrastructureSchedule.supportedAssetTypes);
                
                // Set slaTerms from template defaults/metadata
                if (template.metadata) {
                    // Check for legacy auto_assign flags in template metadata
                    if (template.metadata.auto_assign_new_assets !== undefined) {
                        this.slaTerms.auto_assign_new_assets = template.metadata.auto_assign_new_assets;
                    }
                    if (template.metadata.auto_assign_assets !== undefined) {
                        this.slaTerms.auto_assign_assets = template.metadata.auto_assign_assets;
                    }
                    if (template.metadata.auto_assign_contacts !== undefined) {
                        this.slaTerms.auto_assign_contacts = template.metadata.auto_assign_contacts;
                    }
                    if (template.metadata.auto_assign_new_contacts !== undefined) {
                        this.slaTerms.auto_assign_new_contacts = template.metadata.auto_assign_new_contacts;
                    }
                    if (template.metadata.require_manual_approval !== undefined) {
                        this.slaTerms.require_manual_approval = template.metadata.require_manual_approval;
                    }
                    if (template.metadata.notify_on_assignment !== undefined) {
                        this.slaTerms.notify_on_assignment = template.metadata.notify_on_assignment;
                    }
                }
                
                // Also check template.default_values for sla_terms
                if (template.default_values?.sla_terms) {
                    Object.keys(template.default_values.sla_terms).forEach(key => {
                        if (this.slaTerms.hasOwnProperty(key)) {
                            this.slaTerms[key] = template.default_values.sla_terms[key];
                        }
                    });
                }
            } else {
                ...
                // Clear billing model for custom contracts
                this.pricingSchedule.billingModel = '';
                console.log('Cleared billing model for custom contract');
                
                // Set default supported asset types for custom contracts (all types)
                this.infrastructureSchedule.supportedAssetTypes = ['workstation', 'server', 'network_device', 'hypervisor_node', 'storage', 'printer'];
                console.log('Set default supported asset types for custom contract:', this.infrastructureSchedule.supportedAssetTypes);
        ...
        getSupportedAssetTypesForTemplate(template) {
            if (!template) return [];
            
            const templateType = template.template_type;
            
            // Map template types to supported asset types
            const assetTypeMapping = {
                // MSP Templates - support all asset types
                ...
                'cloud_migration': ['workstation', 'server', 'storage', 'hypervisor_node'],
                'm365_management': ['workstation', 'server'],
                'break_fix': ['workstation', 'server', 'network_device', 'printer'],
                'enterprise_managed': ['workstation', 'server', 'network_device', 'hypervisor_node', 'storage'],
                'mdr_services': ['workstation', 'server', 'network_device', 'security_device'],
                
                // VoIP Templates - minimal infrastructure requirements
                'hosted_pbx': ['workstation'],
                'sip_trunking': ['workstation', 'server', 'network_device'],
                'unified_communications': ['workstation', 'server'],
                'international_calling': ['workstation'],
                'contact_center': ['workstation', 'server'],
                'e911_services': ['workstation'],
                'number_porting': ['workstation'],
                
                // VAR Templates - depends on what's being procured
                'hardware_procurement': ['workstation', 'server', 'network_device', 'storage', 'printer'],
                'software_licensing': ['workstation', 'server'],
                'vendor_partner': ['workstation', 'server', 'network_device'],
                'solution_integration': ['workstation', 'server', 'network_device', 'hypervisor_node'],
                
                // Compliance Templates - all asset types for comprehensive coverage
                'professional_services': ['workstation', 'server', 'network_device'],
                'business_associate': ['workstation', 'server'],
                'data_processing': ['workstation', 'server'],
                
                // General Templates
                'consumption_based': ['workstation', 'server', 'network_device', 'hypervisor_node', 'storage']
            };
            
            // Default to common MSP asset types if template type not found
            return assetTypeMapping[templateType] || ['workstation', 'server', 'network_device'];
        },
        ...
        handleClientSelected(event) {
            ...
            if (event.detail && event.detail.client) {
                ...
                this.fetchAvailableAssets(); // Fetch assets when client is selected
                this.saveProgress();
            }
        },
        ...
        saveProgress() {
            const sanitizedInfrastructureSchedule = JSON.parse(JSON.stringify(this.infrastructureSchedule));
            if (sanitizedInfrastructureSchedule.coverageRules) {
                delete sanitizedInfrastructureSchedule.coverageRules.autoAssignNewAssets;
            }
            const progressData = {
                ...
                infrastructureSchedule: sanitizedInfrastructureSchedule,
                pricingSchedule: this.pricingSchedule,
                additionalTerms: this.additionalTerms,
                slaTerms: this.slaTerms,
            ...
            if (saved) {
                try {
                    ...
                    if (dataAge < 24 * 60 * 60 * 1000) {
                        ...
                        this.slaTerms = { ...this.slaTerms, ...data.slaTerms };
                        this.templateFilter = { ...this.templateFilter, ...data.templateFilter };
                        
                        // Migration: sync old autoAssignNewAssets field to slaTerms.auto_assign_new_assets
                        // Only migrate when slaTerms.auto_assign_new_assets is undefined (not explicitly set)
                        const hasSlaAutoAssign = data.slaTerms && Object.prototype.hasOwnProperty.call(data.slaTerms, 'auto_assign_new_assets');
                        if (!hasSlaAutoAssign && data.infrastructureSchedule?.coverageRules?.autoAssignNewAssets !== undefined) {
                            this.slaTerms.auto_assign_new_assets = data.infrastructureSchedule.coverageRules.autoAssignNewAssets;
                        }
                        
                        // Migration: sync legacy billingConfig flags to slaTerms
                        if (!hasSlaAutoAssign && data.billingConfig?.auto_assign_new_assets !== undefined) {
                            this.slaTerms.auto_assign_new_assets = data.billingConfig.auto_assign_new_assets;
                        }
                        
                        // Migrate auto_assign_assets if not already set in slaTerms
                        const hasSlaAssignAssets = data.slaTerms && Object.prototype.hasOwnProperty.call(data.slaTerms, 'auto_assign_assets');
                        if (!hasSlaAssignAssets && data.billingConfig?.auto_assign_assets !== undefined) {
                            this.slaTerms.auto_assign_assets = data.billingConfig.auto_assign_assets;
                        }
                        
                        // Migrate auto_assign_contacts if not already set in slaTerms
                        const hasSlaAssignContacts = data.slaTerms && Object.prototype.hasOwnProperty.call(data.slaTerms, 'auto_assign_contacts');
                        if (!hasSlaAssignContacts && data.billingConfig?.auto_assign_contacts !== undefined) {
                            this.slaTerms.auto_assign_contacts = data.billingConfig.auto_assign_contacts;
                        }
                        
                        // Migrate auto_assign_new_contacts if not already set in slaTerms
                        const hasSlaAssignNewContacts = data.slaTerms && Object.prototype.hasOwnProperty.call(data.slaTerms, 'auto_assign_new_contacts');
                        if (!hasSlaAssignNewContacts && data.billingConfig?.auto_assign_new_contacts !== undefined) {
                            this.slaTerms.auto_assign_new_contacts = data.billingConfig.auto_assign_new_contacts;
                        }
                        
                        // After migration assignments, remove legacy properties from runtime state
                        if (this.infrastructureSchedule?.coverageRules && 'autoAssignNewAssets' in this.infrastructureSchedule.coverageRules) {
                            delete this.infrastructureSchedule.coverageRules.autoAssignNewAssets;
                        }

                        // Optionally clean legacy flags on billingConfig to avoid sending them
                        ['auto_assign_new_assets','auto_assign_assets','auto_assign_contacts','auto_assign_new_contacts']
                            .forEach(k => { if (k in this.billingConfig) delete this.billingConfig[k]; });
                    }
                } catch (e) {
        ...
        handleSubmission(event) {
            ...
            // Final cleanup to avoid submitting legacy fields
            if (this.infrastructureSchedule?.coverageRules && 'autoAssignNewAssets' in this.infrastructureSchedule.coverageRules) {
                delete this.infrastructureSchedule.coverageRules.autoAssignNewAssets;
            }
        ...
        async fetchAvailableAssets() {
            ...
            try {
                ...
                this.availableAssets = Array.isArray(assets) ? assets : [];
            } catch (error) {
        ...
        getAvailableAssetCount() {
            if (!this.availableAssets || !Array.isArray(this.availableAssets)) {
                console.log('getAvailableAssetCount: No available assets or not array');
                return 0;
            }

            const supportedTypes = this.infrastructureSchedule.supportedAssetTypes || [];
            console.log('getAvailableAssetCount: supportedTypes =', supportedTypes);
            console.log('getAvailableAssetCount: availableAssets =', this.availableAssets);
            
            // If no asset types are configured yet, show count of all unassigned assets
            if (supportedTypes.length === 0) {
                const unassignedAssets = this.availableAssets.filter(asset => !asset.supporting_contract_id);
                console.log('getAvailableAssetCount: No types configured, unassigned assets =', unassignedAssets.length);
                return unassignedAssets.length;
            }

            // Debug: Log actual asset types in your database
            const actualTypes = [...new Set(this.availableAssets.map(asset => asset.type))];
            console.log('getAvailableAssetCount: Actual asset types in database =', actualTypes);
            
            // Debug: Check each asset's type and supporting_contract_id
            const unassignedAssets = this.availableAssets.filter(asset => !asset.supporting_contract_id);
            console.log('getAvailableAssetCount: Unassigned assets =', unassignedAssets.length);
            console.log('getAvailableAssetCount: Sample unassigned asset types =', unassignedAssets.slice(0, 5).map(a => ({ name: a.name, type: a.type, supporting_contract_id: a.supporting_contract_id })));
            
            // Map RMM asset types to contract system types
            const mapAssetType = (assetType) => {
                const typeMap = {
                    // Case variations
                    'Server': 'server',
                    'Desktop': 'workstation', // Map RMM Desktop to workstation
                ...
                return typeMap[assetType] || assetType.toLowerCase();
            };

            // Filter by supported types and only unassigned assets
            const filteredAssets = this.availableAssets.filter(asset => {
                const normalizedAssetType = mapAssetType(asset.type);
                const matches = supportedTypes.includes(normalizedAssetType) && !asset.supporting_contract_id;
                if (matches) {
                    console.log(`Asset "${asset.name}" type "${asset.type}" -> "${normalizedAssetType}" MATCHES`);
                }
                return matches;
            });
            console.log('getAvailableAssetCount: Types configured, filtered assets =', filteredAssets.length);
            return filteredAssets.length;
        },

        getAssetCountByType(assetType) {
            if (!this.availableAssets || !Array.isArray(this.availableAssets)) {
                return 0;
            }
            ...
            return this.availableAssets.filter(asset => {
                const normalizedAssetType = mapAssetType(asset.type);
                return normalizedAssetType === assetType && !asset.supporting_contract_id;
            }).length;
        },

        getAssetsForSchedule() {
            if (!this.availableAssets || !Array.isArray(this.availableAssets)) {
                return [];
            }
            ...
            // For preview purposes, always show unassigned assets
            // If asset types are configured, filter by them; otherwise show all
            let filteredAssets;
            
            if (supportedTypes.length === 0) {
                // No specific asset types configured - show all unassigned assets
                filteredAssets = this.availableAssets.filter(asset => !asset.supporting_contract_id);
            } else {
                // Asset types are configured - filter by supported types and unassigned
                filteredAssets = this.availableAssets.filter(asset => {
                    const normalizedAssetType = mapAssetType(asset.type);
                    return supportedTypes.includes(normalizedAssetType) && 
                           !asset.supporting_contract_id;
                });
            }
        ...
        getAssetsByType() {
            const assets = this.getAssetsForSchedule();
            const assetsByType = {};
