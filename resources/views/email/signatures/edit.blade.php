    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('email.signatures.update', $signature) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <flux:field>
                                ...
                                <flux:input type="text" name="name" id="name" value="{{ old('name', $signature->name) }}" placeholder="e.g., Professional, Casual" required />
                            ...
                            <flux:field>
                                ...
                                <flux:select name="email_account_id" id="email_account_id">
                                    ...
                                    @foreach($accounts as $account)
                                        <option value="{{ $account->id }}" {{ old('email_account_id', $signature->email_account_id) == $account->id ? 'selected' : '' }}>
                            ...
                            <flux:field>
                                ...
                                <flux:checkbox name="is_default" id="is_default" {{ old('is_default', $signature->is_default) ? 'checked' : '' }} />
                                <flux:description>
                        ...
                        <flux:field>
                            ...
                            <textarea
                                ...
                                required
                            >{{ old('content_html', $signature->content_html) }}</textarea>
                            <flux:description>
                        ...
                        <flux:field>
                            ...
                            >{{ old('content_text', $signature->content_text) }}</textarea>
                            <flux:description>
                        ...
                        <div class="mt-6">
                            ...
                            <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900 min-h-[100px]">
                                <div id="signature-preview" class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                                    {!! nl2br(e(old('content_html', $signature->content_html))) !!}
                                </div>
                            </div>
                        </div>
...
<parameter name="todos">[{"content":"Build email account management (add/edit/delete accounts, OAuth/app passwords)","status":"completed","priority":"medium","id":"webmail-account-management"},{"content":"Build email composition interface with rich text, attachments, and templates","status":"completed","priority":"high","id":"webmail-compose-interface"},{"content":"Add email signature management and auto-insertion","status":"completed","priority":"low","id":"webmail-signatures-templates"}]