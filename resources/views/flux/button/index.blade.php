...
$validVariants = ['primary', 'filled', 'outline', 'danger', 'ghost', 'subtle'];
if (!in_array($variant, $validVariants)) {
    \Log::error('Invalid button variant detected', [
        'variant' => $variant,
        'color' => $color,
        'type' => $type,
        'request' => request()->url(),
    ]);
    $variant = 'outline'; // Fallback to outline for invalid variants
}

// Validate and normalize size
$validSizes = ['base', 'sm', 'xs'];
if (!in_array($size, $validSizes)) {
    \Log::error('Invalid button size detected', [
        'size' => $size,
        'variant' => $variant,
        'request' => request()->url(),
    ]);
    $size = 'base'; // Fallback to base for invalid sizes
}

// Normalize color to ensure it's a valid Tailwind color
$validColors = ['slate', 'gray', 'zinc', 'neutral', 'stone', 'red', 'orange', 'amber', 'yellow', 'lime', 'green', 'emerald', 'teal', 'cyan', 'sky', 'blue', 'indigo', 'violet', 'purple', 'fuchsia', 'pink', 'rose'];
if ($color && !in_array($color, $validColors)) {
    \Log::error('Invalid button color detected', [
        'color' => $color, 
        'variant' => $variant, 
        'type' => $type,
        'request' => request()->url(),
    ]);
    $color = null; // Set to null so the match doesn't execute
}
...
$classes = Flux::classes()
    ...
    ->add($inset ? match ($size) { // Inset...
        ...
        default => Flux::applyInset($inset, top: '-mt-2.5', right: '-me-4', bottom: '-mb-3', left: '-ms-4'),
    } : '')
    ->add(match ($variant) { // Background color...
        ...
        default => 'bg-zinc-800/5 hover:bg-zinc-800/10 dark:bg-white/10 dark:hover:bg-white/20',
    })
    ...
    ->add($loading ? [ // Loading states...
     ...
     ->add($variant === 'primary' && $color ? match ($color) {
        ...
        default => '[--color-accent:var(--color-zinc-800)] [--color-accent-content:var(--color-zinc-800)] [--color-accent-foreground:var(--color-white)] dark:[--color-accent:var(--color-white)] dark:[--color-accent-content:var(--color-white)] dark:[--color-accent-foreground:var(--color-zinc-800)]',
    } : '')
    ;
