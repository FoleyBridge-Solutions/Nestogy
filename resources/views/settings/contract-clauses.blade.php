    ...
    <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full sm:p-6">
            <form action="{{ route('settings.contract-clauses.store') }}" method="POST">
                ...
                <div class="mb-6">
                    <label for="content" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Clause Content *</label>
                    <textarea name="content" id="content" rows="10" required 
                              ...
                              placeholder="Enter the clause content... Use @{{variable_name}} for template variables"></textarea>
                    <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                        Use <code>@{{variable_name}}</code> for template variables and <code>@{{#if condition}}...@{{/if}}</code> for conditional content.
                    </p>
                </div>
                ...
                <div class="mb-6" x-data="{ showDefinitions: false }">
                    ...
                    <div class="mt-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg" 
                         x-data="{ selectedDefinitions: [] }" 
                         x-init="
                            $watch('selectedDefinitions', value => {
                                const summary = document.getElementById('definition-summary');
                                if (value.length > 0) {
                                    summary.innerHTML = '<strong>Selected:</strong> ' + value.length + ' definition(s)';
                                    summary.classList.remove('text-gray-500');
                                    summary.classList.add('text-blue-600', 'dark:text-blue-400');
                                } else {
                                    summary.innerHTML = 'No definitions selected';
                                    summary.classList.remove('text-blue-600', 'dark:text-blue-400');
                            ...
                            document.addEventListener('change', function(e) {
                                if (e.target.name === 'required_definitions[]') {
                                    selectedDefinitions = Array.from(document.querySelectorAll('input[name=\"required_definitions[]\"]:checked')).map(cb => cb.value);
                                }
                            });
                         ">
...
function editClause(clauseId) {
    // For now, redirect to a simple edit approach
    // In a real implementation, you'd load the clause data into a modal
    alert('Edit functionality would be implemented here. For now, use duplicate and modify.');
}
</script>
@endsection