...
<div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:px-6">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Switch Client</h1>
                ...
                <div class="flex space-x-3">
                    ...
                    @if($currentClient)
                        <a href="{{ route('clients.clear-selection') }}" class="inline-flex items-center px-4 py-2 border border-red-300 rounded-md shadow-sm text-sm font-medium text-red-700 bg-white hover:bg-red-50">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @if($currentClient)
        <div class="bg-blue-50 border border-blue-200 rounded-lg">
            <div class="px-4 py-5 sm:px-6">
                <h3 class="text-lg font-medium text-blue-900 mb-4">Current Client</h3>
                <div class="flex items-center justify-between bg-white rounded-lg p-4 shadow-sm border border-blue-100">
                    <div class="flex items-center">
                        ...
                        <div class="ml-4">
                            <div class="text-lg font-medium text-gray-900">{{ $currentClient->name }}</div>
                    ...
                    <div class="flex space-x-3">
                        <a href="{{ route('clients.show', $currentClient) }}" class="inline-flex items-center px-3 py-2 border border-transparent text-sm font-medium rounded-md text-blue-700 bg-blue-100 hover:bg-blue-200">
                            View Dashboard
                        </a>
    ...
    <div class="bg-white shadow rounded-lg">
        ...
        <div class="divide-y divide-gray-200">
            @forelse($clients->where('id', '!=', $currentClient?->id ?? 0) as $client)
                <div class="px-6 py-4 hover:bg-gray-50 cursor-pointer client-flex flex-wrap -mx-4" 
                     onclick="selectClient({{ $client->id }}, '{{ addslashes($client->name) }}')">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10">
                                <div class="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center">
                                    <span class="text-sm font-medium text-gray-700">{{ substr($client->name, 0, 2) }}</span>
                                </div>
                            </div>
                            <div class="ml-4">
            ...
            @empty
                <div class="px-6 py-8 text-center">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No other clients available</h3>
                    <p class="mt-1 text-sm text-gray-500">
                        @if($currentClient)
