        ...
        <div class="flex-1 px-6-12">
            <div class="flex justify-between items-center mb-6">
                ...
                <div class="px-6 py-2 font-medium rounded-md transition-colors-group">
                    <a href="{{ route('clients.quotes.standalone.create') }}" class="inline-flex items-center px-6 py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        ...
                        <div class="flex flex-wrap -mx-4 g-3 mt-2">
                            ...
                            <div class="flex-1 px-6-md-2">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <label class="flex items-center-label" for="expiring_soon">
                                        Expiring Soon
                                    </label>
                                </div>
                            </div>
                            ...
                            <div class="flex-1 px-6-md-2">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <label class="flex items-center-label" for="follow_up_due">
                                        Follow-up Due
                                    </label>
                                </div>
                            </div>
                ...
                <div class="p-6">
                                ...
                                <tbody>
                                    @foreach($quotes as $quote)
                                        <tr>
                                            <td>
                                                <div>
                                                    <div class="font-bold">{{ $quote->quote_number }}</div>
                                            ...
                                            <td>
                                                <div>
                                                    <div class="font-bold">{{ $quote->formatted_total_amount }}</div>
                                                    ...
                                                    @endif
                                                    <div class="small text-gray-600 dark:text-gray-400">{{ $quote->currency }}</div>
                                                </div>
                                            </td>
                                            <td>
                                                @switch($quote->status)
                                                    @case('draft')
                                                        ...
                                                        @break
                                                    @case('pending')
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-warning">Pending</span>
                                                        @break
                                                    @case('sent')
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-600">Sent</span>
                                                        @break
                                                    @case('viewed')
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-info">Viewed</span>
                                                        @break
                                                    @case('accepted')
                                                    ...
                                                    @case('cancelled')
                                                        ...
                                                        @break
                                                    @default
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary">{{ ucfirst($quote->status) }}</span>
                                                @endswitch
                                            </td>
                                            <td>
                                                <div class="small">
                                                    @if($quote->valid_until)
                                                        ...
                                                        @if($quote->days_until_expiry !== null)
                                                            @if($quote->days_until_expiry < 0)
                                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-danger">Expired {{ abs($quote->days_until_expiry) }}d ago</span>
                                                            ...
                                                            @elseif($quote->days_until_expiry <= 7)
                                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-info">{{ $quote->days_until_expiry }}d left</span>
                                                            @else
                                                                <span class="text-gray-600 dark:text-gray-400">{{ $quote->days_until_expiry }}d left</span>
                                                            @endif
                                                        @endif
                                                    @else
                                                        <span class="text-gray-600 dark:text-gray-400">No expiry</span>
                                                    @endif
                                            ...
                                            <td>
                                                <div class="small text-center">
                                                    @if($quote->conversion_probability > 0)
                                                        ...
                                                        <div class="font-bold">{{ $quote->conversion_probability }}%</div>
                                                    @else
                                                        <span class="text-gray-600 dark:text-gray-400">Not set</span>
                                                    @endif
                                                </div>
                                            </td>
                                            <td>
                                                <div class=" px-6 py-2 font-medium rounded-md transition-colors-group-sm">
                                                    ...
                                                    @if($quote->isAccepted())
                                                        <button type="button" 
                                                                class="btn px-6 py-2 font-medium rounded-md transition-colors-outline-info" 
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="text-gray-600 dark:text-gray-400">Get started by creating your first quote.</p>
                            <a href="{{ route('clients.quotes.standalone.create') }}" class="btn px-6 py-2 font-medium rounded-md transition-colors-primary">
                                <i class="fas fa-plus mr-2"></i>Add New Quote
                            </a>
                        </div>
                    @endif
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto-content">
            ...
            <div class="fixed inset-0 z-50 overflow-y-auto-footer">
                ...
                <form id="convertQuoteForm" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="inline-flex items-center px-6 py-2 bg-green-600 text-white font-medium rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Convert to Invoice</button>
                </form>
            </div>
        </div>
    </div>
