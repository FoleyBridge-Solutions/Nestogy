...
<div class="container mx-auto mx-auto px-6 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <flux:subheading>
            Manage server racks and equipment placement for {{ $client->name }}
        </flux:subheading>

        <div class="flex gap-3">
            <flux:button variant="outline" href="{{ route('clients.racks.export', $client) }}">
                Export CSV
            </flux:button>
            <flux:button href="{{ route('clients.racks.create', $client) }}">
                Add Rack
            </flux:button>
        </div>
    </div>
    ...
    <flux:table>
        <flux:table.columns>
            <flux:table.column>Rack Details</flux:table.column>
        ...
        <flux:table.rows>
            ...
            <flux:table.row>
                <flux:table.cell>
                    ...
                    <div class="text-sm text-gray-500">Rack #{{ $rack->rack_number }}</div>
                    @if($rack->serial_number)
                        <div class="text-sm text-gray-500">{{ $rack->serial_number }}</div>
                    @endif
                </flux:table.cell>
                <flux:table.cell>
                    <div class="font-medium">{{ $rack->location }}</div>
                    @if($rack->room)
                        <div class="text-sm text-gray-500">{{ $rack->room }}</div>
                    @endif
                </flux:table.cell>
                <flux:table.cell>
                    <flux:badge color="{{ $rack->status === 'active' ? 'green' : ($rack->status === 'maintenance' ? 'yellow' : 'gray') }}">
                        {{ ucfirst($rack->status) }}
                    </flux:badge>
                </flux:table.cell>
                <flux:table.cell>
                    <div class="text-sm">
                        {{ $rack->used_units ?? 0 }}/{{ $rack->total_units ?? 0 }} units
                    </div>
                    @if($rack->total_units)
                        <div class="w-full bg-gray-200 rounded-full h-2 mt-1">
                            <div class="bg-blue-600 h-2 rounded-full" style="width: {{ (($rack->used_units ?? 0) / $rack->total_units) * 100 }}%"></div>
                        </div>
                    @endif
                </flux:table.cell>
                <flux:table.cell>
                    <flux:button variant="ghost" size="sm" href="{{ route('clients.racks.show', [$client, $rack]) }}">
                        View
                    </flux:button>
                    <flux:button variant="ghost" size="sm" href="{{ route('clients.racks.edit', [$client, $rack]) }}">
                        Edit
                    </flux:button>
                </flux:table.cell>
            </flux:table.row>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 p-6">
                    @foreach($racks as $rack)
                        ...
                        <div class="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow duration-200 
                            ...
                            <!-- Description -->
                            @if($rack->description)
                                <p class="text-sm text-gray-600 mb-6 line-clamp-2">{{ $rack->description }}</p>
                            @endif
