    ...
    @if($alerts->isNotEmpty())
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            @foreach($alerts as $alert)
                @php
                    $alertContainerClass = match($alert['severity']) {
                        ...
                        'medium' => 'border-yellow-200 bg-yellow-50/50 dark:border-yellow-800 dark:bg-yellow-900/20',
                        default => 'border-blue-200 bg-blue-50/50 dark:border-blue-800 dark:bg-blue-900/20'
                    };
                @endphp
                <div class="group relative p-3 rounded-lg border transition-all {{ $alertContainerClass }}" wire:key="alert-{{ $alert['id'] }}">
                    
                    <div class="flex items-start gap-3">
                        <!-- Icon -->
                        @php
                            $iconBgClass = match($alert['severity']) {
                        ...
                        @endphp
                        <div class="flex-shrink-0 p-1.5 rounded-full {{ $iconBgClass }}">
                            @php
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                @if($alert['action_url'])
                                    @php
                                        $linkColorClass = match($alert['severity']) {
                                            'critical' => 'text-red-600 dark:text-red-400',
                                            'high' => 'text-orange-600 dark:text-orange-400',
                                            'medium' => 'text-yellow-600 dark:text-yellow-400',
                                            default => 'text-blue-600 dark:text-blue-400'
                                    ...
                                    <a href="{{ $alert['action_url'] }}" 
                                       class="text-xs font-medium hover:underline {{ $linkColorClass }}">
                                        {{ $alert['action_text'] }}
                                    </a>
                                @endif
