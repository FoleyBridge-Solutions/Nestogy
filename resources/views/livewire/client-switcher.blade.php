<div class="relative">
    @if($this->currentClient)
        ...
        <flux:dropdown position="bottom" align="start" width="360">
            <button 
                type="button" 
            ...
            >
                ...
                <div class="flex-1 text-left">
                    ...
                    @if($this->currentClient->company_name && $this->currentClient->company_name !== $this->currentClient->name)
                        <flux:text size="xs" class="text-zinc-500">{{ Str::limit($this->currentClient->company_name, 25) }}</flux:text>
                    @endif
                </div>
            ...
            <flux:popover class="p-4">
                ...
                <div class="flex items-start gap-3 mb-4">
                    <flux:avatar size="lg" :name="$this->currentClient->name" />
                ...
                <div class="space-y-3">
                                ...
                                @foreach($this->favoriteClients as $client)
                                    @if($client->id !== $this->currentClient->id)
                                        <button 
                                            ...
                                            class="w-full flex items-center gap-3 px-2 py-2 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors text-left group"
                                        >
                                            <flux:avatar size="xs" :name="$client->name" />
                                            <div class="flex-1 min-w-0">
                                                <flux:text size="sm" class="font-medium truncate">{{ $client->name }}</flux:text>
                                                @if($client->company_name && $client->company_name !== $client->name)
                                                    <flux:text size="xs" class="text-zinc-500 truncate">{{ $client->company_name }}</flux:text>
                                                @endif
                                            </div>
                                            <flux:icon name="star" class="w-3 h-3 text-yellow-500 fill-current opacity-0 group-hover:opacity-100 transition-opacity" />
                                        </button>
                                    @endif
                                @endforeach
                            </div>
                                    ...
                                    @if($client->id !== $this->currentClient->id)
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                class="p-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-700 opacity-0 group-hover:opacity-100 transition-opacity"
                                                title="Add to favorites"
                                            >
                                                <flux:icon name="star" class="w-3 h-3 text-zinc-400" />
                                            </button>
                                        </button>
                                    @endif
                                @endforeach
                    ...
                    <flux:button variant="ghost" size="sm" icon="eye" href="{{ route('clients.index') }}" class="w-full">
                        View All Clients
                    </flux:button>
                </div>
            </flux:popover>
