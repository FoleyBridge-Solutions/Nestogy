                    ...
                    @if($companies->count() > 0)
                        <form method="POST" action="{{ route('company.set') }}">
                                    ...
                                    <div class="md:w-1/2 px-4 mb-3">
                                        <div class="card company-bg-white rounded-lg shadow-md overflow-hidden h-100" style="cursor: pointer;" onclick="selectCompany({{ $company->id }})">
                                            <div class="p-6 text-center">
                                                ...
                                                <h5 class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ $company->name }}</h5>
                                                @if($company->description)
                                                    <p class="card-text text-gray-600">{{ $company->description }}</p>
                                                @endif
                                                <div class="flex items-center">
                                                    <input class="flex items-center-input" type="radio" name="company_id" value="{{ $company->id }}" id="company_{{ $company->id }}">
                                                    <label class="flex items-center-label" for="company_{{ $company->id }}">
                            ...
                            <div class="grid gap-2 mt-4">
                                <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 px-6 py-3 text-lg" id="continueBtn" disabled>
