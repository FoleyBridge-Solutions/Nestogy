...
<div class="dynamic-contract-list" data-contract-type="{{ $typeConfig['slug'] ?? 'default' }}">
    ...
    <div class="list-header">
        <div class="row align-items-center mb-3">
            <div class="col-md-6">
                <h4 class="mb-0">
                    ...
                    <span class="badge bg-secondary ms-2" id="total-count">
                ...
                <div class="list-actions">
                    ...
                    @if($filterable)
                        <button class="btn btn-outline-primary me-2" data-bs-toggle="collapse" 
                                data-bs-target="#filters-panel" id="toggle-filters">
                            <i class="fas fa-filter"></i> Filters
                            <span class="filter-count badge bg-primary ms-1" style="display: none;">0</span>
                        </button>
                    @endif
                    ...
                    @if($exportable)
                        <div class="btn-group me-2">
                            <button class="btn btn-outline-success dropdown-toggle" 
                                    data-bs-toggle="dropdown" id="export-dropdown">
                        ...
                        @foreach($filters as $filter)
                            <div class="col-md-3">
                                ...
                                @if($filter['type'] === 'select')
                                    <select name="filter[{{ $filter['key'] }}]" class="form-select filter-input">
                                ...
                                @elseif($filter['type'] === 'daterange')
                                    <div class="input-group">
                                        <input type="date" name="filter[{{ $filter['key'] }}][from]" 
                                               class="form-control filter-input" placeholder="From" />
                                        <span class="input-group-text">to</span>
                                        <input type="date" name="filter[{{ $filter['key'] }}][to]" 
                                               class="form-control filter-input" placeholder="To" />
                                    </div>
                                @else
        ...
        @if(!empty($availableBulkActions))
            <div class="bulk-actions-bar alert alert-info" style="display: none;" id="bulk-actions-bar">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="bulk-actions">
                        @foreach($availableBulkActions as $actionKey => $action)
                            <button class="btn btn-{{ $action['color'] }} btn-sm me-2" 
    ...
    <div class="table-container">
        <div class="table-responsive">
            <table class="table {{ $compactMode ? 'table-sm' : '' }}" id="{{ $tableId }}">
                    ...
                    <tr>
                        ...
                        @foreach($displayColumns as $column)
                            <th @if($column['width']) width="{{ $column['width'] }}" @endif
                                ...
                                @endif>
                                <div class="d-flex align-items-center justify-content-{{ $column['align'] === 'center' ? 'center' : ($column['align'] === 'right' ? 'end' : 'start') }}">
                                    {{ $column['label'] }}
                                    @if($column['sortable'])
                                    ...
                                    <td class="text-{{ $column['align'] }}">
                                        @php
                                            ...
                                            if ($column['format'] && is_callable($column['format'])) {
                                                ...
                                                $displayValue = '<span class="badge bg-' . $color . '">' . ($statusConfig['label'] ?? $value) . '</span>';
                        ...
                        <tr>
                            <td colspan="{{ count($displayColumns) + (!empty($availableBulkActions) ? 1 : 0) + (!empty($availableRowActions) ? 1 : 0) }}" 
                                ...
                                <div class="empty-state">
                                    ...
                                    <h5 class="text-muted">No {{ $typeConfig['plural_label'] ?? 'contracts' }} found</h5>
                                    <p class="text-muted">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function setupSorting() {
            ...
            table.addEventListener('click', function(e) {
                ...
                icon.className = `fas fa-sort-${newDirection === 'asc' ? 'up' : 'down'} text-primary ms-1 sort-icon`;
        ...
        function loadTableData(queryString) {
            ...
            tbody.innerHTML = '<tr><td colspan="100%" class="text-center py-4"><i class="fas fa-spinner fa-spin"></i> Loading...</td></tr>';
            ...
            fetch(url)
                ...
                .catch(error => {
                    ...
                    tbody.innerHTML = '<tr><td colspan="100%" class="text-center py-4 text-danger"><i class="fas fa-exclamation-triangle"></i> Error loading data</td></tr>';
                });
        }
            ...
            fetch(`{{ route('contracts.stats') }}?type={{ $typeConfig['slug'] ?? 'default' }}`)
                ...
                .then(data => {
                    const statsHtml = Object.entries(data).map(([key, value]) => {
                        ...
                        return `<span class="me-3"><strong>${value}</strong> ${label}</span>`;
                    }).join('');
                    
                    statsContainer.innerHTML = `<small class="text-muted">${statsHtml}</small>`;
                })
                .catch(error => {
