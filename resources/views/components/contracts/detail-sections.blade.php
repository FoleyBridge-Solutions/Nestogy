...
@php
    ...
    function formatFieldValue($value, $field, $contract) {
        ...
        switch ($type) {
            ...
            case 'boolean':
                return $value ? 
                    '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-success">Yes</span>' : 
                    '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary">No</span>';
                    
            case 'status':
                $statusConfig = $typeConfig['statuses'][$value] ?? [];
                $color = $statusConfig['color'] ?? 'secondary';
                $label = $statusConfig['label'] ?? $value;
                return '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-' . $color . '">' . $label . '</span>';
                    ...
                    if ($user) {
                        return '<div class="flex items-center">
                                    <img src="' . ($user->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&size=24') . '" 
                                         class="rounded-full mr-2" width="24" height="24" alt="' . $user->name . '">
                                    ' . $user->name . '
                                </div>';
                    }
                }
            ...
            case 'tags':
                if (is_array($value)) {
                    return implode(' ', array_map(function($tag) {
                        return '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-light text-dark border">' . $tag . '</span>';
                    }, $value));
                } elseif (is_string($value)) {
                    $tags = explode(',', $value);
                    return implode(' ', array_map(function($tag) {
                        return '<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-light text-dark border">' . trim($tag) . '</span>';
                    }, $tags));
                }
                return $value;
...
@foreach($sections as $sectionKey => $section)
    <div class="contract-section mb-6" id="section-{{ $sectionKey }}">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-header {{ $section['collapsible'] ?? false ? 'cursor-pointer' : '' }}" 
                 @if($section['collapsible'] ?? false) 
                      
                     
                 @endif>
                <h6 class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-title mb-0 flex items-center">
                    ...
                    @if($section['badge'] ?? false)
                        <span class="badge bg-{{ $section['inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium']['color'] ?? 'primary' }} ml-2">
                            {{ $section['badge']['text'] }}
                        </span>
                    @endif
                </h6>
            ...
            <div class="{{ $section['collapsible'] ?? false ? 'collapse show' : '' }}" 
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-body">
                    ...
                    @if($section['layout'] ?? 'grid' === 'table')
                        ...
                        <div class="min-w-full divide-y divide-gray-200 dark:divide-gray-700-responsive">
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 min-w-full divide-y divide-gray-200 dark:divide-gray-700-sm">
                                <thead>
                                    <tr>
                                        <th scope="col-span-12" style="width: 30%;">Field</th>
                                ...
                                @foreach($section['fields'] as $fieldConfig)
                                    ...
                                    <tr>
                                        <td class="fw-medium text-gray-600 dark:text-gray-400" style="width: 30%;">{{ $fieldLabel }}:</td>
                                        <td>
                    ...
                    @elseif($section['layout'] ?? 'grid' === 'list')
                        ...
                        <div class="divide-y divide-gray-200 dark:divide-gray-700 divide-y divide-gray-200 dark:divide-gray-700-flush">
                            @foreach($section['fields'] as $fieldConfig)
                                ...
                                <div class="divide-y divide-gray-200 dark:divide-gray-700-item px-0">
                                        ...
                                        @if(is_array($fieldConfig) && ($fieldConfig['actions'] ?? false))
                                            <div class="field-actions">
                                                @foreach($fieldConfig['actions'] as $action)
                                                    <a href="{{ $action['url'] ?? '#' }}" 
                                                       class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-{{ $action['color'] ?? 'primary' }}"
                                                       title="{{ $action['label'] }}">
                        ...
                        <div class="flex flex-wrap -mx-4 g-3">
                            @foreach($section['fields'] as $fieldConfig)
                                @php
                                    ...
                                    $fieldValue = data_get($contract, $fieldKey);
                                    $fieldType = is_array($fieldConfig) ? ($fieldConfig['type'] ?? 'text') : 'text';
                                    $colSize = is_array($fieldConfig) ? ($fieldConfig['col_size'] ?? 'md:w-1/2') : 'md:w-1/2';
                                ...
                                <div class="{{ $colSize }}">
                                    <div class="field-group">
                                        <label class="field-label text-gray-600 dark:text-gray-400 small">{{ $fieldLabel }}</label>
                                        <div class="field-value">
                                            {!! formatFieldValue($fieldValue, is_array($fieldConfig) ? $fieldConfig : ['type' => $fieldType], $contract) !!}
                                        </div>
                                        
                                        @if(is_array($fieldConfig) && ($fieldConfig['help_text'] ?? false))
                                            <small class="form-text text-gray-600 dark:text-gray-400">{{ $fieldConfig['help_text'] }}</small>
                                        @endif
                                        ...
                                        @if(is_array($fieldConfig) && ($fieldConfig['actions'] ?? false))
                                            <div class="field-actions mt-1">
                                                @foreach($fieldConfig['actions'] as $action)
                                                    <a href="{{ $action['url'] ?? '#' }}" 
                                                       class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-{{ $action['color'] ?? 'primary' }} mr-1"
                                                       title="{{ $action['label'] }}">
                                                        <i class="{{ $action['icon'] ?? 'fas fa-link' }}"></i>
                                                        {{ $action['label'] ?? '' }}
                                                    </a>
                    ...
                    @if($section['actions'] ?? false)
                        <div class="section-actions mt-6 pt-3 border-t">
                            @foreach($section['actions'] as $action)
                                @if($action['type'] ?? 'link' === 'button')
                                    <button class="btn px-6 py-2 font-medium rounded-md transition-colors-{{ $action['color'] ?? 'primary' }} mr-2"
                                ...
                                @else
                                    <a href="{{ $action['url'] ?? '#' }}" 
                                       class="btn px-6 py-2 font-medium rounded-md transition-colors-{{ $action['color'] ?? 'primary' }} mr-2"
...
<style>
    ...
    /* List group styling */
    .divide-y divide-gray-200 dark:divide-gray-700-flush .divide-y divide-gray-200 dark:divide-gray-700-item {
        border-left: 0;
        border-right: 0;
    }
    
    .divide-y divide-gray-200 dark:divide-gray-700-flush .divide-y divide-gray-200 dark:divide-gray-700-item:first-child {
        border-top: 0;
    }
    
    .divide-y divide-gray-200 dark:divide-gray-700-flush .divide-y divide-gray-200 dark:divide-gray-700-item:last-child {
        border-bottom: 0;
    }
</style>
@endpush
