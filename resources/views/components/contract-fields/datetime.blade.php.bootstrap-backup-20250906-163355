...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function validateDateTime(date) {
            ...
            const container = datetimeInput.closest('.form-group');
            let errorDiv = container.querySelector('.datetime-validation-errors');
            
            if (errors.length > 0) {
                if (!errorDiv) {
                    errorDiv = document.createElement('div');
                    errorDiv.className = 'datetime-validation-errors text-danger small mt-1';
        ...
        quickTimesContainer.innerHTML = '<small class="text-muted">Quick times:</small> ';
        ...
        datetimeInput.closest('.form-group').appendChild(quickTimesContainer);
        @endif
    });
</script>
@endpush