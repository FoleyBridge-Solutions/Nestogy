...
@php
    ...
    $textareaClass = 'block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm' . ($hasError ? ' border-red-500' : '');
...
<div class="mb-4 mb-6">
    ...
    @if($wysiwyg)
        <div class="wysiwyg-container mx-auto">
            <div id="{{ $fieldSlug }}_toolbar" class="wysiwyg-toolbar">
                <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" data-command="bold" title="Bold">
                    <i class="fas fa-bold"></i>
                </button>
                <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" data-command="italic" title="Italic">
                    <i class="fas fa-italic"></i>
                </button>
                <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" data-command="underline" title="Underline">
                    <i class="fas fa-underline"></i>
                </button>
                <div class="px-6 py-2 font-medium rounded-md transition-colors-group">
                    <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary dropdown-toggle"  title="Format">
                ...
                <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" data-command="insertUnorderedList" title="Bullet List">
                    <i class="fas fa-list-ul"></i>
                </button>
                <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" data-command="insertOrderedList" title="Numbered List">
    ...
    @if($showCharCount && $maxlength)
        <small class="form-text text-gray-600 dark:text-gray-400 flex justify-between">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        @if($showCharCount && $maxlength)
            ...
            function updateCharCount() {
                ...
                if (percentage > 90) {
                    charCountElement.className = 'text-red-600 dark:text-red-400';
                } else if (percentage > 75) {
                    charCountElement.className = 'text-yellow-600 dark:text-yellow-400';
                } else {
                    charCountElement.className = 'text-gray-600 dark:text-gray-400';
                }
            }
