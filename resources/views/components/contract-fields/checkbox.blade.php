...
@php
    ...
    $checkboxClass = 'flex items-center-input';
    if ($size === 'sm') $checkboxClass .= ' flex items-center-input-sm';
    if ($size === 'lg') $checkboxClass .= ' flex items-center-input-lg';
    if ($hasError) $checkboxClass .= ' border-red-500';
@endphp
...
<div class="mb-4 mb-6">
    ...
    <div class="{{ $containerClass }}" 
        ...
        @if($selectAll && !$isSingle)
            {{-- Select All option --}}
            <div class="flex items-center {{ $checkboxStyle === 'switch' ? 'form-switch' : '' }} select-all-option">
                <input 
                    type="checkbox"
                    id="{{ $fieldSlug }}_select_all"
                    class="flex items-center-input select-all-checkbox"
                />
                <label class="flex items-center-label font-bold" for="{{ $fieldSlug }}_select_all">
                    Select All
                </label>
            </div>
            <hr class="my-2">
        ...
        @if($isSingle)
            {{-- Single checkbox --}}
            <div class="flex items-center {{ $checkboxStyle === 'switch' ? 'form-switch' : '' }}">
                <input 
                    type="hidden"
                    name="{{ $fieldSlug }}"
                    value="0"
                ...
                />
                <label class="flex items-center-label" for="{{ $fieldSlug }}">
                    {{ $field['label'] }}
                    @if($field['is_required'])
                        <span class="text-red-600 dark:text-red-400">*</span>
                    @endif
        ...
        @else
            ...
            @foreach($formattedOptions as $index => $option)
                ...
                @if($checkboxStyle === 'button')
                    ...
                    <div class="flex items-center-button">
                        <input 
                            ...
                            id="{{ $optionId }}"
                            name="{{ $fieldSlug }}[]"
                            class="px-6 py-2 font-medium rounded-md transition-colors-check checkbox-option"
                            value="{{ $option['value'] }}"
                ...
                @else
                    ...
                    <div class="flex items-center {{ $checkboxStyle === 'switch' ? 'form-switch' : '' }} {{ $inline ? 'flex items-center-inline' : '' }}">
                        <input 
                            type="checkbox"
                            id="{{ $optionId }}"
                            name="{{ $fieldSlug }}[]"
...
<style>
    ...
    .checkbox-columns .flex items-center {
        break-inside: avoid;
        margin-bottom: 0.5rem;
    }
    ...
    .form-switch-{{ $colorName }} .flex items-center-input:checked {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        @if(!$isSingle)
            ...
            function validateRequired() {
                ...
                const container = checkboxContainer.closest('.mb-4');
                let errorDiv = container.querySelector('.checkbox-required-error');
                
                if (checkedCount === 0) {
                    if (!errorDiv) {
                        errorDiv = document.createElement('div');
                        errorDiv.className = 'checkbox-required-error text-red-600 text-sm mt-1 block';
                    ...
                    optionCheckboxes.forEach(cb => cb.classList.add('border-red-500'));
                } else {
                    if (errorDiv) {
                        errorDiv.remove();
                    }
                    ...
                    optionCheckboxes.forEach(cb => cb.classList.remove('border-red-500'));
                }
            }
