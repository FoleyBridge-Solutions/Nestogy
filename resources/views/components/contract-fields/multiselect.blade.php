...
<div class="mb-4 mb-6">
    <label for="{{ $fieldSlug }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
        ...
        @endif
        @if($showCount)
            <span class="selection-count inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary ml-2" id="{{ $fieldSlug }}_count">0 selected</span>
        @endif
    </label>
    
    <div class="multiselect-container mx-auto">
        @if($showSelectAll || $showClearAll)
            <div class="multiselect-controls mb-2">
                @if($showSelectAll)
                    <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-primary mr-2" id="{{ $fieldSlug }}_select_all">
                        <i class="fas fa-check-double"></i> Select All
                    </button>
                @endif
                @if($showClearAll)
                    <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" id="{{ $fieldSlug }}_clear_all">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const tomSelect = new TomSelect(selectElement, {
            ...
            render: {
                option: function(item, escape) {
                    ...
                    // Icon
                    if (item.icon) {
                        html += `<i class="${escape(item.icon)} mr-2"></i>`;
                    }
                    
                    // Color indicator
                    if (item.color) {
                        html += `<span class="color-indicator mr-2" style="background-color: ${escape(item.color)}; width: 12px; height: 12px; border-radius: 2px; display: inline-block;"></span>`;
                    }
                ...
                item: function(item, escape) {
                    ...
                    if (item.icon) {
                        html += `<i class="${escape(item.icon)} mr-1"></i>`;
                    }
                    
                    if (item.color) {
                        html += `<span class="color-indicator mr-1" style="background-color: ${escape(item.color)}; width: 8px; height: 8px; border-radius: 2px; display: inline-block;"></span>`;
                    }
        ...
        function updateSelectionCount() {
            @if($showCount)
                ...
                if (countElement) {
                    ...
                    countElement.className = `selection-count badge ml-2 ${count > 0 ? 'bg-primary' : 'bg-secondary'}`;
                }
            @endif
        }
        ...
        function validateSelection() {
            ...
            if (errors.length > 0) {
                if (!errorDiv) {
                    ...
                    errorDiv.className = 'multiselect-validation-errors text-red-600 dark:text-red-400 small mt-1';
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                const errorDiv = selectElement.closest('.mb-4').querySelector('.multiselect-validation-errors');
                if (errorDiv) {
                    e.preventDefault();
                    selectElement.focus();
                    return false;
