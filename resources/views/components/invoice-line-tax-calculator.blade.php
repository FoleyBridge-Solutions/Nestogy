...
<div id="line-tax-calculator-{{ $lineItem->id ?? 'new' }}" class="line-tax-calculator">
    ...
    <div class="flex flex-wrap -mx-4 mb-6">
        <div class="md:w-full px-6">
            <div class="bg-white rounded-lg shadow-md overflow-hidden border-info">
                <div class="px-6 py-6 border-b border-gray-200 bg-gray-50 bg-gray-100">
                    <h6 class="mb-0">
                        ...
                        Tax Jurisdiction
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-info ml-2" id="tax-jurisdiction-confidence">{{ $customer ? 'Calculated' : 'Unknown' }}</span>
                    </h6>
                </div>
                <div class="p-6 py-2">
                    @if($customer)
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-gray-600">{{ $customer->city }}, {{ $customer->state }} {{ $customer->zip_code }}</span>
                        </div>
                    @else
                        <div class="text-gray-600">Customer address required for tax calculation</div>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if($product && $product->requiresTaxData())
        <div class="bg-white rounded-lg shadow-md overflow-hidden border-primary mb-6">
            <div class="px-6 py-6 border-b border-gray-200 bg-gray-50 bg-blue-600 text-white">
                <h6 class="mb-0">
                    <i class="fas fa-list-check"></i>
                    Tax Calculation Information
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-dark ml-2">{{ $product->taxProfile->name ?? 'Standard' }}</span>
                </h6>
            </div>
            <div class="p-6">
                <div class="flex flex-wrap -mx-4" id="dynamic-tax-fields-{{ $lineItem->id ?? 'new' }}">
                        ...
                        @if($fieldType === 'number')
                            <div class="md:w-1/2 px-6 mb-6">
                                <label for="{{ $fieldId }}" class="block text-sm font-medium text-gray-700 mb-1">
                                    ...
                                    @if($fieldHelp)
                                        <i class="fas fa-info-circle text-gray-600 dark:text-gray-400 ml-1" 
                                           x-data x-tooltip 
                                           title="{{ $fieldHelp }}"></i>
                                    @endif
                                </label>
                                <input type="number" 
                                       class="block w-full px-6 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm line-tax-field" 
                                       id="{{ $fieldId }}" 
                                       ...
                                       required>
                                <div class="text-red-600 text-sm mt-1">Please enter a valid {{ strtolower($fieldLabel) }}</div>
                            </div>
                            ...
                            <div class="flex-1 px-6-md-12 mb-6">
                                ...
                                <div class="flex flex-wrap -mx-4">
                                    ...
                                    <div class="flex-1 px-6-md-3">
                                        <input type="text" 
                                               class="block w-full px-6 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm line-tax-field" 
                                               placeholder="State"
                                               id="{{ $fieldId }}_state" 
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden border-success" id="line-tax-results-{{ $lineItem->id ?? 'new' }}" style="display: none;">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-header bg-success text-white">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center">
                    <div class="spinner-border spinner-border-sm mr-2" id="line-tax-loading-{{ $lineItem->id ?? 'new' }}" style="display: none;"></div>
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-light text-dark" id="line-tax-engine-{{ $lineItem->id ?? 'new' }}">{{ $product->getTaxEngineType() ?? 'Standard' }}</span>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden-body">
            ...
            <div class="flex flex-wrap -mx-4">
                <div class="flex-1 px-6-md-8">
                    <div class="line-tax-summary p-6 bg-light rounded">
                        <div class="flex justify-between items-center mb-2">
                            <span>Line Subtotal ({{ $quantity }} × ${{ number_format($unitPrice, 2) }}):</span>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <strong class="h5 mb-0 text-green-600">$<span id="line-total-{{ $lineItem->id ?? 'new' }}">{{ number_format($quantity * $unitPrice, 2) }}</span></strong>
                        </div>
                    </div>
                </div>
                <div class="flex-1 px-6-md-4">
                    <div class="tax-metadata">
                        <h6 class="text-gray-600 dark:text-gray-400 mb-2">Tax Details</h6>
            ...
            <div class="mt-6" id="line-tax-breakdown-section-{{ $lineItem->id ?? 'new' }}" style="display: none;">
                <div class="flex items-center justify-between mb-2">
                    <h6 class="text-gray-600 dark:text-gray-400 mb-0">
                        <i class="fas fa-chart-pie"></i>
                        Tax Breakdown
                    </h6>
                    <button type="button" class="btn px-4 py-1 text-sm px-6 py-2 font-medium rounded-md transition-colors-outline-secondary" id="toggle-line-breakdown-{{ $lineItem->id ?? 'new' }}">
...
class InvoiceLineTaxCalculator {
    ...
    initializeTooltips() {
        var tooltipTriggerList = [].slice.call(this.container.querySelectorAll('[x-data x-tooltip]'));
    ...
    validateField(field) {
        ...
        if (isRequired && !value) {
            ...
            field.classList.remove('border-red-500', 'is-valid');
        }
    }
    ...
    displayTaxBreakdown(breakdown) {
        ...
        for (const [code, tax] of Object.entries(breakdown)) {
            ...
            item.className = 'flex justify-between mb-1 p-2 bg-light rounded';
            item.innerHTML = `
                <span><strong>${tax.name}</strong> (${tax.rate}%)</span>
                <span class="text-green-600">$${tax.amount.toFixed(2)}</span>
            `;
