    ...
    <form action="{{ $action }}" method="POST" class="p-6" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($fields as $field)
                <div class="{{ $field['width'] ?? 'flex-1 px-6-span-1' }}">
                    ...
                    @switch($field['type'] ?? 'text')
                        ...
                        @case('file')
                            <input type="file" 
                                   ...
                                   class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-6 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                            @break
        ...
        <div class="flex items-center justify-end mt-6 pt-6 border-t border-gray-200 space-x-3">
            @if($cancelRoute)
                <a href="{{ $cancelRoute }}" 
                   class="px-6 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-150">
                    {{ $cancelText }}
                </a>
            @endif
