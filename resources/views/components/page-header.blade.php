...
<div class="{{ $compact ? '' : 'mb-4' }}">
    <div class="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border-b border-gray-200 dark:border-gray-700">
        <div class="{{ $compact ? 'px-4 py-2 sm:px-6' : 'px-4 py-3 sm:px-6' }}">
            <div class="flex items-center justify-between">
                ...
                @if($actions || $backRoute)
                    <div class="ml-4 flex items-center space-x-2">
                        @if($backRoute)
                            <a href="{{ $backRoute }}" 
                               class="inline-flex items-center {{ $compact ? 'px-2.5 py-1 text-xs' : 'px-3 py-1.5 text-sm' }} border border-gray-300 dark:border-gray-600 rounded-md font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 transition-colors">
