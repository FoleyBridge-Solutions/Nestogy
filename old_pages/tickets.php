...
if (isset($_GET['assigned']) & !empty($_GET['assigned'])) {
    $ticket_assigned_filter = 'AND ticket_assigned_to = '.$user_id;
    // Unassigned
    ...
    $ticket_assigned_filter = 'AND (ticket_assigned_to = 0 OR ticket_assigned_to = ' . $user_id . ')';
}
...
$sql_total_tickets_assigned = mysqli_query($mysqli, "SELECT COUNT(ticket_id) AS total_tickets_assigned FROM tickets WHERE ticket_assigned_to = $user_id AND ticket_status != '5'");
