            ...
            <div class="card-datatable table-responsive  pt-0">                
                <table class="datatables-basic table border-top">
                    ...
                    <tr>
                        ...
                        <?php if ($mobile) { ?> <th>Tags</th> <?php } ?>
                        <th data-priority="2">Primary Location</th>
                        <th>Primary Contact</th>
                        <?php if (($user_role == 3 || $user_role == 1) && $config_module_enable_accounting == 1) { ?> <th class="text-right " data-priority="3">Billing</th> <?php } ?>
                    ...
                    while ($row = mysqli_fetch_array($sql)) {
                        ...
                        <tr>
                            ...
                            <?php } ?>
                            <td>
                                <a href="//maps.<?= $map_source; ?>.com/?q=<?= urlencode($location_address.' '.$location_zip) ?>" target="_blank">
                                    <?= $location_address_display; ?>
                                </a>
                            </td>
                            <td>
                            ...
                            <?php if (($user_role == 3 || $user_role == 1) && $config_module_enable_accounting == 1) { ?>
                                <td class="text-right">
                                    <div class="mt-1">
                                        <span class="text-secondary">Balance</span> <span class="<?= $balance_text_color; ?>"><?= numfmt_format_currency($currency_format, $balance, $company_currency); ?></span>
                                    </div>
                                    <div class="mt-1">
                                        <span class="text-secondary">Paid</span> <?= numfmt_format_currency($currency_format, $amount_paid, $company_currency); ?>
                                    </div>
                                    <div class="mt-1">
                                        <span class="text-secondary">Monthly</span> <?= numfmt_format_currency($currency_format, $recurring_monthly, $company_currency); ?>
                                    </div>
                                    <div class="mt-1">
                                        <span class="text-secondary">Hourly Rate</span> <?= numfmt_format_currency($currency_format, $client_rate, $company_currency); ?>
                                    </div>
                                </td>
                            <?php } ?>
