...
$sql_recent_logins = mysqli_query($mysqli, "SELECT * FROM logs
    WHERE log_type = 'Login' OR log_type = 'Login 2FA' AND log_action = 'Success' AND log_user_id = $user_id
    ORDER BY log_id DESC LIMIT 3"
);

$sql_recent_logs = mysqli_query($mysqli, "SELECT * FROM logs
    WHERE log_user_id = $user_id AND log_type NOT LIKE 'Login'
...
<div class="card">
    ...
    <div class="card-footer">
        <a href="admin_logs.php?q=<?= "$name successfully logged in"; ?>">See More...</a>
    </div>
</div>
...
<div class="card">
    ...
    <div class="card-footer">
        <a href="admin_logs.php?q=<?= nullable_htmlentities($name); ?>">See More...</a>
    </div>
</div>
