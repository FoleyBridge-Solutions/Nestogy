...
echo "🔍 Searching for ALL O'Connor addresses in 78247...\n\n";
...
while (($data = fgetcsv($handle)) !== FALSE) {
    if (count($data) >= 27) {
        ...
        if ($street === "O'CONNOR" && $zip === '78247') {
            $foundAddresses[] = [
                'from' => $from,
                'to' => $to,
                'covers_17422' => ($from <= 17422 && $to >= 17422),
                'county_taid' => $data[18],
                'city_taid' => $data[19], 
...
echo "📊 Found " . count($foundAddresses) . " O'Connor addresses in 78247:\n\n";

foreach ($foundAddresses as $addr) {
    $covers = $addr['covers_17422'] ? '✅ COVERS 17422' : '❌ no';
    echo "🏠 Range: {$addr['from']}-{$addr['to']} {$covers}\n";
    if ($addr['covers_17422']) {
