...
class ProjectControllerRefactored extends BaseController
{
    protected function initializeController(): void
    {
        ...
        $this->eagerLoadRelations = ['client', 'manager', 'members.user'];
    }

    protected function getFilters(Request $request): array
    {
        return $request->only([
            'search', 'status', 'priority', 'category', 'client_id', 
            'manager_id', 'member_id', 'date_range'
        ]);
    }
    ...
    protected function getIndexViewData(Request $request): array
    {
        ...
        if ($request->get('view') === 'timeline') {
            return $this->getTimelineViewData($request);
        }

        if ($request->get('view') === 'kanban') {
            return $this->getKanbanViewData($request);
        }

        return compact('clients', 'managers', 'members', 'statistics');
    }
    ...
    protected function getShowViewData(\Illuminate\Database\Eloquent\Model $model): array
    {
        ...
        return compact(
            'project', 'statistics', 'recentTasks', 'recentMilestones',
            'healthStatus', 'upcomingDeadlines', 'timelineData'
        );
    }
    ...
    protected function getCreateViewData(): array
    {
        ...
        return compact('clients', 'managers', 'templates');
    }
    ...
    public function updateStatus(Request $request, Project $project)
    {
        ...
        try {
            ...
            $this->logActivity($project, 'status_changed', $request, [
            ...
            $this->logError('status_update', $e, $request, $project);
    ...
    public function clone(Request $request, Project $project)
    {
        ...
        try {
            $newProject = DB::transaction(function () use ($project, $request) {
            ...
            $this->logActivity($project, 'cloned', $request, [
            ...
            $this->logError('clone', $e, $request, $project);
    ...
    public function export(Request $request)
    {
        ...
        $filters = $this->getFilters($request);
        $query = $this->buildIndexQuery($request);
        $projects = $query->with(['client', 'manager'])->get();
        ...
        $callback = function() use ($projects) {
            ...
            // CSV headers
            fputcsv($file, [
                'Project Code', 'Project Name', 'Status', 'Priority', 'Category',
                'Client', 'Manager', 'Start Date', 'Due Date', 'Progress %',
                'Budget', 'Actual Cost', 'Team Size', 'Tasks Count',
                'Health Status', 'Created At'
            ]);
    ...
    protected function getTimelineViewData(Request $request): array
    {
        $query = $this->buildIndexQuery($request);
        $projects = $query->with(['tasks', 'milestones'])->get();
    ...
    protected function getKanbanViewData(Request $request): array
    {
        $query = $this->buildIndexQuery($request);
        $projects = $query->get();
        ...
        return ['kanbanData' => $kanbanData, 'kanbanColumns' => $kanbanColumns];
    }
}