...
class TaxCloudApiClient extends BaseApiClient
{
        ...
        return $this->makeRequest(
            ...
            function () use ($address, $cartItems) {
                ...
                if (isset($data['ResponseType']) && $data['ResponseType'] === 'Error') {
                    throw new Exception('TaxCloud API error: '.($data['Messages'][0] ?? self::UNKNOWN_ERROR_MESSAGE));
                }
    ...
    public function verifyAddress(array $address): array
    {
        ...
        return $this->makeRequest(
            ...
            function () use ($address) {
                ...
                if (isset($data['ResponseType']) && $data['ResponseType'] === 'Error') {
                    throw new Exception('TaxCloud verification error: '.($data['Messages'][0] ?? self::UNKNOWN_ERROR_MESSAGE));
                }
    ...
    public function getJurisdictionTaxInfo(string $state, string $zip): array
    {
        ...
        return $this->makeRequest(
            ...
            function () use ($state, $zip) {
                ...
                if (! $lookup['success']) {
                    throw new Exception('Failed to get jurisdiction info: '.($lookup['error'] ?? self::UNKNOWN_ERROR_MESSAGE));
                }
    ...
    protected function getOriginAddress(): array
    {
        ...
        return [
            'Address1' => config('services.company.address1'),
            'Address2' => config('services.company.address2'),
            'City' => config('services.company.city'),
            'State' => config('services.company.state'),
            'Zip5' => config('services.company.zip5'),
            'Zip4' => config('services.company.zip4'),
        ];
    }
