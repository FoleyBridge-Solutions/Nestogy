...
class ContractClauseService
{
    ...
    protected function removeDuplicateHeaders(string $content, string $sectionHeader, string $category): string
    {
        ...
        // Extract the section title without numbering (e.g., "1. SCOPE OF SUPPORT SERVICES" -> "SCOPE OF SUPPORT SERVICES")
        $sectionTitle = preg_replace('/^\d+\.\s*/', '', $sectionHeader);
        
        // Common variations of how headers might appear in content
        $headerPatterns = [
            // Exact match at start of content
            '/^' . preg_quote($sectionTitle, '/') . '\s*:?\s*\n*/i',
            // With colon at start
            '/^' . preg_quote($sectionTitle, '/') . '\s*:\s*\n*/i',
            // Standalone line
            '/^' . preg_quote($sectionTitle, '/') . '\s*\n+/im',
            // With trailing colon on its own line
            '/^' . preg_quote($sectionTitle, '/') . '\s*:?\s*$/im',
        ];
        ...
        // Clean up any leading whitespace or newlines left after header removal
