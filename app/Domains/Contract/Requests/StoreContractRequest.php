...
class StoreContractRequest extends FormRequest
{
     ...
     * Note: sla_terms is decoded here to enable nested array validation and differs
     * from other JSON fields handled in validatedWithComputed().
     */
    protected function prepareForValidation(): void
    {
        $sla = $this->input('sla_terms');
        $normalized = $this->normalizeSlaTerms($sla);
        $this->merge(['sla_terms' => $normalized]);
    }
    ...
    protected function validateTemplateSpecificRequirements($validator): void
    {
        ...
        try {
            $template = \App\Domains\Contract\Models\ContractTemplate::where('company_id', $this->user()->company_id)
    ...
    /**
     * Get parsed SLA terms from JSON, always returns an array for validation contexts
     */
    protected function getParsedSlaTerms(): array
    {
        return $this->parseSlaTermsInternal($this->input('sla_terms'), false);
    }
    ...
    public function validatedWithComputed(): array
    {
        ...
        if (! empty($validated['variable_values'])) {
            $validated['variable_values'] = is_string($validated['variable_values'])
                ? (json_decode($validated['variable_values'], true) ?? [])
                : (is_array($validated['variable_values']) ? $validated['variable_values'] : []);
        }
        if (! empty($validated['billing_config'])) {
            $validated['billing_config'] = is_string($validated['billing_config'])
                ? (json_decode($validated['billing_config'], true) ?? [])
                : (is_array($validated['billing_config']) ? $validated['billing_config'] : []);
        }
        if (! empty($validated['infrastructure_schedule'])) {
            $validated['infrastructure_schedule'] = is_string($validated['infrastructure_schedule'])
                ? (json_decode($validated['infrastructure_schedule'], true) ?? [])
                : (is_array($validated['infrastructure_schedule']) ? $validated['infrastructure_schedule'] : []);
        }
        if (! empty($validated['pricing_schedule'])) {
            $validated['pricing_schedule'] = is_string($validated['pricing_schedule'])
                ? (json_decode($validated['pricing_schedule'], true) ?? [])
                : (is_array($validated['pricing_schedule']) ? $validated['pricing_schedule'] : []);
        }
        if (! empty($validated['additional_terms'])) {
            $validated['additional_terms'] = is_string($validated['additional_terms'])
                ? (json_decode($validated['additional_terms'], true) ?? [])
                : (is_array($validated['additional_terms']) ? $validated['additional_terms'] : []);
        }
    ...
    /**
     * Normalize sla_terms value from string JSON to array, returns null for empty/invalid input
     */
    private function normalizeSlaTerms($value)
    {
        return $this->parseSlaTermsInternal($value, true);
    }

    /**
     * Internal helper to parse SLA terms with configurable fallback behavior
     *
     * @param  mixed  $value  The value to parse (string JSON, array, or other)
     * @param  bool  $emptyAsNull  If true, returns null for empty/invalid input; if false, returns []
     * @return array|null Returns parsed array, null (if emptyAsNull=true), or [] (if emptyAsNull=false)
     */
    private function parseSlaTermsInternal($value, bool $emptyAsNull = true)
    {
        if (is_string($value)) {
            $trim = trim($value);
