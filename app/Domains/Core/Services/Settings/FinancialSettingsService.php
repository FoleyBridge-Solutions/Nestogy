...
class FinancialSettingsService extends BaseSettingsService
{
    ...
    protected function getValidationRules(string $category): array
    {
        switch ($category) {
            ...
            case 'invoicing':
                return [
                    'invoice_footer' => self::NULLABLE_STRING . '|max:1000',
                    'invoice_notes' => self::NULLABLE_STRING . '|max:1000',
                    'show_tax_id' => 'boolean',
                    'show_payment_instructions' => 'boolean',
                    'payment_instructions' => self::NULLABLE_STRING . '|max:1000',
                    'attach_pdf' => 'boolean',
                    'allow_partial_payments' => 'boolean',
                    'minimum_payment_amount' => 'nullable|numeric|min:0',
                    'invoice_template' => self::NULLABLE_STRING,
                ];

            case 'taxes':
                return [
                    'tax_enabled' => 'boolean',
                    'tax_name' => self::NULLABLE_STRING . '|max:50',
                    'tax_rate' => 'nullable|numeric|between:0,100',
                    'tax_number' => self::NULLABLE_STRING . '|max:50',
                    'tax_compound' => 'boolean',
                    'tax_on_shipping' => 'boolean',
                    'tax_inclusive' => 'boolean',
                    'voip_tax_enabled' => 'boolean',
                    'voip_tax_api_key' => self::NULLABLE_STRING,
                ];

            case 'payment_gateways':
                return [
                    'stripe_enabled' => 'boolean',
                    'stripe_publishable_key' => self::NULLABLE_STRING,
                    'stripe_secret_key' => self::NULLABLE_STRING,
                    'paypal_enabled' => 'boolean',
                    'paypal_client_id' => self::NULLABLE_STRING,
                    'paypal_secret' => self::NULLABLE_STRING,
