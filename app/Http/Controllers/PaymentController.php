...
class PaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = [
            'total_amount' => Payment::where('company_id', $companyId)->where('status', 'completed')->sum('amount'),
            'this_month_amount' => Payment::where('company_id', $companyId)
                ->where('status', 'completed')
                ->whereMonth('payment_date', now()->month)
                ->whereYear('payment_date', now()->year)
                ->sum('amount'),
            'pending_count' => Payment::where('company_id', $companyId)->where('status', 'pending')->count(),
            'failed_count' => Payment::where('company_id', $companyId)->where('status', 'failed')->count()
        ];
