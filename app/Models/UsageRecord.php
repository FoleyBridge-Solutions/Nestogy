...
class UsageRecord extends Model
{
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($record) {
            $record->setTransactionIdIfNeeded();
            $record->setUsageTimingAttributes();
            $record->calculateDurationIfNeeded();
            $record->setBillingPeriodIfNeeded();
        });
    ...
    private function setTransactionIdIfNeeded(): void
    {
    ...
    private function setUsageTimingAttributes(): void
    {
        ...
        $this->is_peak_time = $this->isPeakHour($this->usage_start_time->hour);
    ...
    private function calculateDurationIfNeeded(): void
    {
        $hasBothTimes = $this->usage_start_time && $this->usage_end_time;
        
        if ($hasBothTimes && ! $this->duration_seconds) {
            $this->duration_seconds = $this->usage_end_time->diffInSeconds($this->usage_start_time);
        }
    }

    private function setBillingPeriodIfNeeded(): void
    {
        $shouldSetBillingPeriod = ! $this->billing_period 
            && \Schema::hasColumn('usage_records', 'billing_period');

        if ($shouldSetBillingPeriod) {
            $this->billing_period = now()->format('Y-m');
        }
    }

    private function updateProcessedTimestamp(): void
    {
        $isNowProcessed = $this->isDirty('processing_status') 
