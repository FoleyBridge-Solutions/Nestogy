...
class CommandPalette extends Component
{
    ...
    private function getPopularCommands()
    {
        ...
        if ($user && ! $user instanceof \App\Models\Contact) {
            $commands = $this->getFavoriteOrPopularActions($user);
        }
    ...
    private function getFavoriteOrPopularActions($user)
    {
        ...
        $commands = $this->formatActionsAsCommands($favoriteActions, true);

        if (empty($commands)) {
            $popularActions = QuickActionService::getPopularActions($user);
            $commands = $this->formatActionsAsCommands($popularActions, false);
        }
    ...
    private function filterFavoriteActions($allActions, $favoriteIds)
    {
        return $allActions->filter(function ($action) use ($favoriteIds) {
            if ($this->isCurrentRoute($action)) {
                $this->logFilteredAction($action);
                return false;
            }
    ...
    private function isCurrentRoute($action)
    {
        return $this->currentRoute && isset($action['route']) && $action['route'] === $this->currentRoute;
    }

    private function logFilteredAction($action)
    {
    ...
    private function formatActionsAsCommands($actions, $isFavorite = false)
    {
        $commands = [];

        foreach ($actions as $action) {
            if (!$isFavorite && $this->isCurrentRoute($action)) {
                continue;
            }

            $commands[] = $this->buildQuickActionCommand($action, $isFavorite);
        }
    ...
    private function shouldSkipNavigationCommand($navCommand, $existingRoutes, $existingTitles)
    {
        ...
        return $this->isSimilarTitle($navCommand['title'], $existingTitles);
    }

    private function isSimilarTitle($navTitle, $existingTitles)
    {
        ...
        foreach ($existingTitles as $existingTitle) {
            ...
            foreach ($keywords as $keyword) {
                if (str_contains($existingTitle, $keyword) && str_contains($navTitleLower, $keyword)) {
                    return true;
                }
            }
        }
