...
class TimerBatchCompletionModal extends Component
{
        ...
        try {
            ...
            foreach ($this->activeTimers as $timer) {
                ...
                if ($settings['add_comment'] && $timer->ticket) {
                    $this->addTimerCompletionComment($commentService, $timer, $result, $settings);
                }
    ...
    protected function getTimerSettings($timerId)
    {
        ...
        $settings = $this->individualSettings[$timerId];

        return [
            'description' => $settings['description'],
            'work_type' => $settings['work_type'],
            'is_billable' => $settings['is_billable'],
            'add_comment' => $settings['add_comment'],
        ];
    }

    protected function addTimerCompletionComment($commentService, $timer, $result, $settings)
    {
        $hours = round($result->hours_worked, 2);
