...
class TemplateContentGenerator
{
    ...
    protected function generateVarServiceContent(Contract $contract, array $variables): string
    {
        ...
        if (!empty($variables['includes_installation'])) $installationServices[] = 'Basic installation';
        if (!empty($variables['includes_rack_stack'])) $installationServices[] = 'Rack and stack services';
        if (!empty($variables['includes_cabling'])) $installationServices[] = 'Cabling and connectivity';
        if (!empty($variables['includes_configuration'])) $installationServices[] = 'System configuration';
        if (!empty($variables['includes_project_management'])) $installationServices[] = 'Project management';
    ...
    protected function generateComplianceServiceContent(Contract $contract, array $variables): string
    {
        ...
        // Audit services
        $auditServices = [];
        if (!empty($variables['includes_internal_audits'])) $auditServices[] = 'Internal compliance audits';
        if (!empty($variables['includes_external_audits'])) $auditServices[] = 'External audit coordination';
        if (!empty($variables['includes_penetration_testing'])) $auditServices[] = 'Penetration testing';
        if (!empty($variables['includes_vulnerability_scanning'])) $auditServices[] = 'Vulnerability scanning';
