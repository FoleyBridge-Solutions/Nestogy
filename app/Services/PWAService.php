...
use EragLaravelPwa\Services\PWAService as BasePWAService;

class PWAService extends BasePWAService
{
    ...
    public function RegisterServiceWorkerScript(): string
    {
        ...
        // FIXED: Removed the <script src="{$swPath}"></script> line
        // Service workers should NOT be loaded as regular scripts
        return <<<HTML
        {$installApp}
        <!-- PWA scripts -->
        <script {$isLivewire}>
            ...
            if ("serviceWorker" in navigator) {
                navigator.serviceWorker.register("{$swPath}").then(
                    (registration) => {
                        {$consoleLog}("Service worker registration succeeded:", registration);
                    },
                    (error) => {
