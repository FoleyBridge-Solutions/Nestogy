...
class TicketPolicy
{
    ...
    {
        // Ticket must not already be resolved or closed
        if ($ticket->is_resolved || $ticket->status === 'Closed') {
            return false;
        }

        // Admins and managers can resolve any ticket
        if ($user->hasRole(['admin', 'manager']) && $user->company_id === $ticket->company_id) {
            return true;
        }

        // Assigned technician can resolve their tickets
        if ($user->hasRole('technician') && $ticket->assigned_to === $user->id) {
            return true;
        }
    ...
    public function reopen(User $user, Ticket $ticket): bool
    {
        ...
        return $ticket->canBeReopenedBy($user);
    }
    ...
    {
        // Only admins and managers can permanently close tickets
        return $user->hasRole(['admin', 'manager']) && $user->company_id === $ticket->company_id;
    }

    /**
     * Determine whether the user can manage the ticket.
     */
    public function manage(User $user, Ticket $ticket): bool
    {
        // Admins and managers can manage any ticket in their company
        return $user->hasRole(['admin', 'manager']) && $user->company_id === $ticket->company_id;
    }
}
