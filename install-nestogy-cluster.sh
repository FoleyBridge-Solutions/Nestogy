...
# ====================================================================
# 🚀 NESTOGY ERP - CLUSTER INSTALLATION SCRIPT v2.0
# ====================================================================
# All-in-One Production Installation for Dedicated Database Clusters
# Supports: External PostgreSQL, MySQL/MariaDB clusters
...
EOF
    echo -e "${NC}"
    print_message "🏢 Cluster Installation Script v2.0" "$YELLOW"
    print_message "📊 External Database • ⚡ High-Performance • 🔒 Secure" "$WHITE"
    echo
}
...
test_postgresql_connection() {
    ...
    if PGPASSWORD="$DB_PASS" psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" >/dev/null 2>&1; then
        print_message "✅ PostgreSQL connection successful" "$GREEN"
    else
        print_message "❌ PostgreSQL connection failed" "$RED"
        print_message "Please check your database credentials and network connectivity" "$YELLOW"
        exit 1
    fi
}
...
test_mysql_connection() {
    ...
    if mysql -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "SELECT 1;" >/dev/null 2>&1; then
        print_message "✅ MySQL/MariaDB connection successful" "$GREEN"
    else
        print_message "❌ MySQL/MariaDB connection failed" "$RED"
        print_message "Please check your database credentials and network connectivity" "$YELLOW"
        exit 1
    fi
}
...
collect_user_input() {
    ...
    echo
    print_message "📊 Database Cluster Configuration:" "$CYAN"
    print_message "Choose your external database type:" "$WHITE"
    print_message "1) PostgreSQL (Recommended)" "$WHITE"
    print_message "2) MySQL/MariaDB" "$WHITE"
    echo
    ...
    while true; do
        ...
        case $db_choice in
            1)
                DB_TYPE="postgresql"
                DB_PORT="5432"
                print_message "✅ PostgreSQL selected" "$GREEN"
                break
                ;;
            2)
                DB_TYPE="mysql"
                DB_PORT="3306"
                print_message "✅ MySQL/MariaDB selected" "$GREEN"
                break
                ;;
            *)
    ...
    echo
    print_message "📋 Enter your database cluster details:" "$CYAN"
    read -p "🏢 Database Host/IP: " DB_HOST
    read -p "🔌 Database Port [$DB_PORT]: " custom_port
    ...
    print_message "Database: $DB_TYPE at $DB_HOST:$DB_PORT" "$WHITE"
    print_message "Database Name: $DB_NAME" "$WHITE"
...
install_database_client() {
    print_section "Installing Database Client"
    
    if [[ "$DB_TYPE" == "postgresql" ]]; then
        print_message "🐘 Installing PostgreSQL client..." "$YELLOW"
        
        # Add PostgreSQL repository for latest client
        ...
        print_message "🗄️  Installing MySQL client..." "$YELLOW"
        apt-get install -y -q mysql-client
    fi
    
    print_message "✅ Database client installed" "$GREEN"
}

# ====================================================================
# WEB SERVER INSTALLATION (SAME AS SINGLE SERVER)
# ====================================================================
...
EOF
    ...
    cp "$fmp_config" "${fpm_config}.backup" 2>/dev/null || true
...
php_admin_value[error_log] = /var/log/fmp-php.www.log
...
configure_application() {
    ...
    # Database configuration for external cluster
    if [[ "$DB_TYPE" == "postgresql" ]]; then
    ...
    print_message "🔍 Testing application database connection..." "$YELLOW"
...
# ====================================================================
# REMAINING FUNCTIONS (SAME AS SINGLE SERVER)
# ====================================================================
...
configure_security() {
    ...
    # Allow database connections if on private network
    print_message "🗄️  Note: Ensure your database cluster allows connections from this server IP: $SERVER_IP" "$CYAN"
...
final_optimizations() {
    ...
    COMPOSER_ALLOW_SUPERUSER=1 composer dump-autoloader --optimize
...
display_summary() {
    print_section "🎉 Cluster Installation Complete!"
    ...
    echo
    print_message "╔════════════════════════════════════════════════════════════════╗" "$CYAN"
    print_message "║                🏢 NESTOGY ERP CLUSTER INSTALLED 🏢             ║" "$CYAN"
    print_message "╚════════════════════════════════════════════════════════════════╝" "$CYAN"
    echo
    ...
    print_message "🗄️  Database: $DB_TYPE at $DB_HOST:$DB_PORT" "$GREEN"
    print_message "📊 Database Name: $DB_NAME" "$GREEN"
    print_message "🐘 PHP Version: $PHP_VERSION" "$GREEN"
    print_message "📦 Node.js Version: $(node -v)" "$GREEN"
    echo
    
    print_message "🏗️  CLUSTER ARCHITECTURE:" "$BLUE"
    print_message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" "$BLUE"
    print_message "✅ Application Server: $SERVER_IP" "$WHITE"
    print_message "✅ Database Cluster: $DB_HOST:$DB_PORT" "$WHITE"
    print_message "✅ Local Redis Cache: 127.0.0.1:6379" "$WHITE"
    print_message "✅ Queue Workers: 4 processes" "$WHITE"
    echo
    ...
    print_message "✅ External database cluster" "$WHITE"
    print_message "✅ Distributed scheduler prevention" "$WHITE"
    ...
    print_message "✅ Database connection encrypted" "$WHITE"
    if [[ -n "$DOMAIN_NAME" ]]; then
    ...
    print_message "Test DB connection: cd $APP_DIR && php artisan tinker --execute=\"DB::select('SELECT 1');\"" "$WHITE"
    print_message "Clear cache: cd $APP_DIR && php artisan cache:clear" "$WHITE"
    echo
    
    print_message "⚠️  IMPORTANT NOTES:" "$YELLOW"
    print_message "━━━━━━━━━━━━━━━━━━━━━━" "$YELLOW"
    print_message "🔒 Ensure your database cluster allows connections from: $SERVER_IP" "$WHITE"
    print_message "🌐 Database should be accessible on: $DB_HOST:$DB_PORT" "$WHITE"
    print_message "🔥 Configure database firewall to allow this app server" "$WHITE"
    print_message "💾 Set up database backups on your cluster" "$WHITE"
    print_message "📊 Monitor database connections and performance" "$WHITE"
    echo
    ...
    print_message "5. 🔄 Test database connectivity thoroughly" "$YELLOW"
    print_message "6. 📊 Set up monitoring for both app and database" "$YELLOW"
    echo
    
    print_message "🎊 Thank you for using Nestogy ERP!" "$GREEN"
    print_message "Your high-performance cluster setup is ready! 🚀" "$GREEN"
    echo
}
...
main() {
    ...
    # Start installation
    print_message "🚀 Starting Nestogy ERP cluster installation..." "$GREEN"
    sleep 2
