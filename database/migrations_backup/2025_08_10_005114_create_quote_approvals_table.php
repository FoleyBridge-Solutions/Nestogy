...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('quote_approvals', function (Blueprint $table) {
            ...
            $table->index(['quote_id', 'approval_level']);
            $table->index(['status', 'approval_level']);
            $table->index('approved_at');
            $table->index('rejected_at');

            // Unique constraint to prevent duplicate approvals at same level for same quote
            $table->unique(['quote_id', 'approval_level'], 'unique_quote_approval_level');
        });
    }
